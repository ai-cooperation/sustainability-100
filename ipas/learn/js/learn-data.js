// iPAS 淨零碳規劃管理師 三問法學習系統 — 模組資料
// 初級 L1：4 模組 / 中級 L2：6 模組

window.LEARN_DATA = {
  "l1": {
    "name": "初級",
    "nameFull": "iPAS 淨零碳規劃管理師（初級）",
    "examFormat": "單選 50 題 × 75 分鐘 × 2 科，70 分及格",
    "focus": "認識制度：名詞、流程、記憶",
    "modules": [
      {
        "id": "L111",
        "name": "國際氣候治理歷程",
        "subject": "淨零碳規劃管理基礎概論",
        "scope": "COP 歷程、重要決議、國內外氣候治理、淨零專有名詞",
        "frameworkQuestion": "從 1992 UNFCCC 到 2015 巴黎協定、再到 2021 格拉斯哥氣候協議，國際氣候治理的三大里程碑分別解決了什麼問題？",
        "controversy": "為什麼巴黎協定是「各國自訂」而非強制減量？這種設計是突破還是妥協？",
        "concepts": [
          { "id": "cop-history", "name": "COP 歷程與里程碑", "tags": ["UNFCCC","京都議定書","巴黎協定","COP26","考古題"] },
          { "id": "ndc-mechanism", "name": "NDC 與減碳機制", "tags": ["NDC","碳預算","台灣淨零","考古題"] },
          { "id": "ghg-science", "name": "溫室氣體科學基礎", "tags": ["IPCC","AR6","碳中和","淨零","考古題"] }
        ],
        "frameworkQ": {
          "question": "從 1992 UNFCCC 到 2015 巴黎協定、再到 2021 格拉斯哥氣候協議，國際氣候治理的三大里程碑分別解決了什麼問題？",
          "hint": "想想看：UNFCCC 建立了什麼框架？京都議定書為什麼只約束部分國家？巴黎協定的 NDC 機制又解決了什麼瓶頸？",
          "answer": "UNFCCC（1992）建立了「共同但有差別責任」的基礎框架；京都議定書（1997）首次設定量化減碳目標，但只約束已開發國家；巴黎協定（2015）改為全體自主貢獻（NDC），解決了開發中國家參與的瓶頸。格拉斯哥（2021）首次明確提到「逐步減少煤炭」。"
        },
        "controversyQ": {
          "scenario": "巴黎協定是「各國自訂」而非強制減量，這種設計是突破還是妥協？",
          "sideA": { "label": "是突破", "argument": "NDC 機制讓 195 個國家全部參與，遠超京都議定書的 37 國。自下而上的承諾比強制分配更有政治可行性，每 5 年提高一次雄心確保持續進步。" },
          "sideB": { "label": "是妥協", "argument": "自訂目標沒有法律約束力，各國可以隨時降低承諾。目前各國 NDC 加總仍不足以控制 1.5°C，證明自願機制力度不夠。" },
          "insight": "NDC 機制是外交上的突破（全球參與）和科學上的妥協（力度不足）。關鍵在於「棘輪機制」（ratchet mechanism）——每 5 年只能提高不能降低雄心，長期來看會逐步收緊。"
        },
        "questions": [
          {
            "id": "L111_Kogu_367",
            "type": "考古題",
            "q": "環境部已於 113 年 2 月 5 日「溫室氣體排放係數」，並 採用 IPCC 第幾次評估所公告之溫室氣體暖化潛勢？",
            "options": [
              "AR4",
              "AR5",
              "AR6",
              "以上皆非"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 1,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L111_Kogu_370",
            "type": "考古題",
            "q": "若企業希望產品進入歐盟市場，碳足跡數據需符合？",
            "options": [
              "WTO 環境貿易協定",
              "CBAM 要求",
              "巴黎協定 COP21",
              "ISO 9001"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 1,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L111_Kogu_374",
            "type": "考古題",
            "q": "IPCC AR5 中，甲烷 (CH₄) 的 100 年 GWP 約為：",
            "options": [
              "21",
              "25",
              "28",
              "34"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 1,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L111_Kogu_375",
            "type": "考古題",
            "q": "IPCC AR6 中，氧化亞氮 (N₂O) 的 100 年 GWP 值約為：",
            "options": [
              "265",
              "273",
              "298",
              "310"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 1,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L111_Kogu_348",
            "type": "考古題",
            "q": "什麼是巴黎協定 (Paris Agreement)？",
            "options": [
              "2015 年簽署的國際環境協議，旨在限制全球升溫",
              "從大氣中移除二氧化碳的技術",
              "2015 年簽署的國際貿易協議，旨在促進自由貿易",
              "促進各國碳交易的市場機制"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 1,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L111_Kogu_372",
            "type": "考古題",
            "q": "哪個國際政策最直接要求產品提供碳足跡數據？",
            "options": [
              "WTO 環保協定",
              "歐盟 CBAM",
              "COP21 巴黎協定",
              "ISO 50001"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 1,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L111_Kogu_192",
            "type": "考古題",
            "q": "坎昆協議規定附件一國家 MRV 範圍，下列何者正確？",
            "options": [
              "強化減緩行動報告（項目與成果）",
              "接受 ICA 機制對 NAMAs 之參與",
              "與目標相關基線資料計算",
              "每兩年更新排放報告並接受 ICA"
            ],
            "correct": 2,
            "explain": "ICA 是非附件一國家使用，附件一國家應使用 IAR (國際評估及審查",
            "epRef": 1,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L111_S01",
            "type": "補充",
            "q": "聯合國氣候變化綱要公約 (UNFCCC) 最早於哪一年通過？",
            "options": [
              "1987 年蒙特婁",
              "1992 年里約",
              "1997 年京都",
              "2005 年蒙特婁"
            ],
            "correct": 1,
            "explain": "UNFCCC 於 1992 年里約地球高峰會通過，是國際氣候治理的基礎框架。",
            "epRef": 1,
            "source": "supplement",
            "tag": ["UNFCCC"]
          },
          {
            "id": "L111_S02",
            "type": "補充",
            "q": "巴黎協定的核心目標是將全球升溫控制在工業化前水平以上多少度以內？",
            "options": [
              "控制在 1.0°C 以內",
              "控制在 1.5°C 以內",
              "控制在 2°C 以內，並努力限制在 1.5°C",
              "控制在 3°C 以內"
            ],
            "correct": 2,
            "explain": "巴黎協定目標為「遠低於 2°C，並努力限制在 1.5°C」，選項 C 最完整。",
            "epRef": 1,
            "source": "supplement",
            "tag": ["巴黎協定"]
          },
          {
            "id": "L111_S03",
            "type": "補充",
            "q": "「國家自定貢獻」(Nationally Determined Contribution, NDC) 機制的核心特色為何？",
            "options": [
              "各國由聯合國統一分配減碳配額",
              "各國自主提出減碳目標，每五年更新一次",
              "僅已開發國家需要提交減碳計畫",
              "各國碳排放量不得超過 1990 年水準"
            ],
            "correct": 1,
            "explain": "NDC 是巴黎協定的核心機制，由各國自主設定目標，每五年逐步提高雄心。",
            "epRef": 1,
            "source": "supplement",
            "tag": ["NDC"]
          },
          {
            "id": "L111_S04",
            "type": "補充",
            "q": "COP26（格拉斯哥氣候協議）首次在 COP 決議文中明確提到要逐步減少的化石燃料是？",
            "options": [
              "石油",
              "天然氣",
              "煤炭",
              "所有化石燃料"
            ],
            "correct": 2,
            "explain": "格拉斯哥氣候協議首次在 COP 決議中明確提到「逐步減少」(phase down) 煤炭使用。",
            "epRef": 33,
            "source": "supplement",
            "tag": ["COP26"]
          },
          {
            "id": "L111_S05",
            "type": "補充",
            "q": "京都議定書與巴黎協定的主要差異為何？",
            "options": [
              "京都議定書涵蓋所有國家，巴黎協定僅涵蓋已開發國家",
              "京都議定書僅約束已開發國家減碳，巴黎協定要求所有締約方提交 NDC",
              "京都議定書採自願減碳，巴黎協定採強制分配",
              "兩者對減碳義務的要求完全相同"
            ],
            "correct": 1,
            "explain": "京都議定書僅對附件一（已開發）國家設定強制目標；巴黎協定改為所有締約方自主貢獻。",
            "epRef": 1,
            "source": "supplement",
            "tag": ["京都議定書","巴黎協定"]
          },
          {
            "id": "L111_S06",
            "type": "補充",
            "q": "台灣「2050 淨零排放路徑」規劃中，下列哪一項不是官方公布的四大轉型策略？",
            "options": [
              "能源轉型",
              "產業轉型",
              "碳交易轉型",
              "生活轉型"
            ],
            "correct": 2,
            "explain": "台灣四大轉型策略為：能源轉型、產業轉型、生活轉型、社會轉型。碳交易非四大轉型之一。",
            "epRef": 41,
            "source": "supplement",
            "tag": ["台灣淨零"]
          },
          {
            "id": "L111_S07",
            "type": "補充",
            "q": "下列何者正確描述「碳中和」(Carbon Neutrality) 與「淨零排放」(Net Zero) 的差異？",
            "options": [
              "兩者完全相同，只是不同語言的翻譯",
              "碳中和僅涵蓋 CO₂，淨零排放涵蓋所有溫室氣體",
              "淨零排放允許使用碳抵換，碳中和不允許",
              "碳中和是企業用語，淨零排放是國家用語"
            ],
            "correct": 1,
            "explain": "碳中和通常指 CO₂ 排放與移除平衡；淨零排放涵蓋所有溫室氣體（CO₂、CH₄、N₂O 等），範圍更廣。",
            "epRef": 37,
            "source": "supplement",
            "tag": ["碳中和","淨零"]
          },
          {
            "id": "L111_S08",
            "type": "補充",
            "q": "IPCC 第六次評估報告 (AR6) 指出，若要控制升溫在 1.5°C 以內，全球溫室氣體排放需在何時達到淨零？",
            "options": [
              "2030 年前",
              "2040 年前",
              "2050 年前後",
              "2070 年前"
            ],
            "correct": 2,
            "explain": "IPCC AR6 指出 1.5°C 情境下，全球需在 2050 年前後達到 CO₂ 淨零排放。",
            "epRef": 1,
            "source": "supplement",
            "tag": ["IPCC","AR6"]
          }
        ],
        "reinforcement": []
      },
      {
        "id": "L112",
        "name": "淨零管理基礎概論",
        "subject": "淨零碳規劃管理基礎概論",
        "scope": "CDP/SBTi/RE100/GRI/SASB/TCFD/IFRS、CBAM、ISO 14068-1、台灣 2050 路徑、碳資產交易",
        "frameworkQuestion": "CDP、SBTi、RE100、TCFD、GRI 這五個字母縮寫，分別屬於「揭露框架」「倡議目標」還是「準則」？",
        "controversy": "ISO 14068-1 碳中和標準跟 PAS 2060 差在哪？為何 ISO 要推新版取代？",
        "concepts": [
          { "id": "disclosure-framework", "name": "揭露框架與倡議", "tags": ["CDP","GRI","SASB","TCFD","IFRS","考古題"] },
          { "id": "target-initiative", "name": "減碳目標與倡議", "tags": ["SBTi","RE100","碳中和","淨零","考古題"] },
          { "id": "carbon-market", "name": "碳市場與交易", "tags": ["碳交易","CBAM","碳風險","考古題"] },
          { "id": "standard-cert", "name": "標準與認證", "tags": ["ISO","PAS2060","LCA","碳足跡","ESG","考古題"] }
        ],
        "frameworkQ": {
          "question": "CDP、SBTi、RE100、TCFD、GRI 這五個字母縮寫，分別屬於「揭露框架」「倡議目標」還是「準則」？",
          "hint": "想想看：哪些是告訴你「要揭露什麼」？哪些是告訴你「目標要設多高」？哪些是告訴你「報告怎麼寫」？",
          "answer": "揭露框架：CDP（碳揭露問卷）、TCFD（氣候財務揭露）。倡議目標：SBTi（科學基礎減碳目標）、RE100（100% 再生能源）。準則：GRI（永續報告準則）。注意 SASB 也是準則（產業別財務重大性），IFRS/ISSB 正在整合 TCFD+SASB。"
        },
        "controversyQ": {
          "scenario": "ISO 14068-1 碳中和標準跟 PAS 2060 差在哪？為何 ISO 要推新版取代？",
          "sideA": { "label": "PAS 2060 夠用", "argument": "PAS 2060 已被廣泛採用超過 10 年，流程成熟（量化→減量→抵換→宣告），企業熟悉度高。替換標準會增加合規成本。" },
          "sideB": { "label": "ISO 14068-1 更嚴謹", "argument": "PAS 2060 對抵換品質要求不夠嚴格，導致「買碳權就碳中和」的漂綠行為。ISO 14068-1 要求更明確的減量優先原則，限制抵換比例，並對碳權品質設門檻。" },
          "insight": "核心差異在「減量優先」的強制程度。PAS 2060 允許大量抵換，ISO 14068-1 要求先證明已盡最大努力減量。趨勢是「抵換≠減量」的共識越來越強，企業不能只靠買碳權宣稱碳中和。"
        },
        "questions": [
          {
            "id": "L112_Kogu_349",
            "type": "考古題",
            "q": "什麼是氣候變遷減緩 (Climate Change Mitigation)？",
            "options": [
              "減緩氣候變遷的行動",
              "研究環境污染的學科",
              "研究如何減少化學品對環境和人類健康影響",
              "以上皆是"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_346",
            "type": "考古題",
            "q": "什麼是綠色物流？",
            "options": [
              "使用環保運輸工具的物流",
              "使用再生能源的物流",
              "減少包裝材料的物流",
              "以上皆是"
            ],
            "correct": 3,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_342",
            "type": "考古題",
            "q": "什麼是淨零排放(Net-zero emissions)？",
            "options": [
              "完全不排放溫室氣體",
              "碳排放量等於碳吸收量",
              "碳排放量低於法規標準",
              "碳排放量逐年下降"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_334",
            "type": "考古題",
            "q": "什麼是碳風險？",
            "options": [
              "企業因氣候變遷導致財務損失的風險",
              "企業因碳排放超標被罰款的風險",
              "企業因投資綠能產業失敗的風險",
              "以上皆是"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_327",
            "type": "考古題",
            "q": "下列何者是 PAS2060 碳中和宣告的必要條 因？",
            "options": [
              "進行溫室氣體盤查",
              "設定減量目標",
              "採取減量行動",
              "以上皆是"
            ],
            "correct": 3,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_369",
            "type": "考古題",
            "q": "哪一項最能代表 Scope 3 排放？",
            "options": [
              "工廠燃煤鍋爐",
              "購買電力",
              "員工通勤、供應鏈物流",
              "自有發電機"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_350",
            "type": "考古題",
            "q": "什麼是碳排放交易系統 (Emissions Trading System, ETS)？",
            "options": [
              "政府對碳排放設定上限並允許企業交易排放",
              "政府對低碳產品提供補貼的政策工具",
              "政府對高碳排放產品徵收關稅的政策工具",
              "以上皆是"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_332",
            "type": "考古題",
            "q": "下列何者不是生命週期評估 (Life Cycle Assessment) 的步驟？",
            "options": [
              "目標與範疇定義",
              "清單分析",
              "氧化亞氮(N2O)",
              "二氧化硫(SO2)"
            ],
            "correct": 3,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_353",
            "type": "考古題",
            "q": "什麼是氣候變遷調適 (Climate Change Adaptation)？",
            "options": [
              "評估企業因氣候變遷導致財務損失的風險",
              "評估社區或生態系統對氣候變遷的敏感性和",
              "評估政府氣候政策的有效性",
              "以上皆非"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_339",
            "type": "考古題",
            "q": "什麼是環境、社會和公司治理 (ESG)？",
            "options": [
              "企業評估投資風險的指標",
              "企業評估永續發展績效的指標",
              "政府評估企業社會責任的指標",
              "以上皆是"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_355",
            "type": "考古題",
            "q": "什麼是氣候變遷減緩 (Climate Change Mitigation)？",
            "options": [
              "減少溫室氣體排放和增加碳匯的行動",
              "適應氣候變遷影響的行動",
              "監測氣候變遷數據的科學研究",
              "評估氣候風險對企業的財務影響"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_344",
            "type": "考古題",
            "q": "什麼是氣候變遷情境分析 (Climate Change Scenario Analysis)？",
            "options": [
              "分析不同氣候變遷情境對企業的影響",
              "分析不同氣候變遷情境對社會的影響",
              "分析不同氣候變遷情境對環境的影響",
              "以上皆是"
            ],
            "correct": 3,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_335",
            "type": "考古題",
            "q": "什麼是氣候變遷調適 (Climate Change Adaptation)？",
            "options": [
              "減緩氣候變遷的行動",
              "適應氣候變遷的行動",
              "預測氣候變遷的行動",
              "使用較多能源產生相同產出的能力"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_326",
            "type": "考古題",
            "q": "什麼是科學基礎減量目標 (Science Based Targets, SBTi)？",
            "options": [
              "企業根據自身產業特性設定的減碳目標",
              "企業根據國際減碳趨勢設定的減碳目標",
              "企業根據科學證據設定的減碳目標，與將全",
              "企業根據政府政策設定的減碳目標"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_360",
            "type": "考古題",
            "q": "什麼是氣候風險(Climate risk)？",
            "options": [
              "企業因氣候變遷導致財務損失的風險",
              "企業因碳排放超標被罰款的風險",
              "企業因投資綠能產業失敗的風險",
              "以上皆是"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          }
        ],
        "reinforcement": [
          {
            "id": "L112_Kogu_362",
            "type": "考古題",
            "q": "在組織邊界外所獲得的數據稱之為",
            "options": [
              "僅報告最終結果",
              "不揭露敏感數據",
              "確保量化結果的全面性和準確性",
              "網路資料庫數據"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_333",
            "type": "考古題",
            "q": "什麼是全球暖化潛勢 (Global Warming Potential, GWP)？",
            "options": [
              "一種溫室氣體吸收熱量的能力",
              "一種溫室氣體在大氣中停留的時間",
              "一種溫室氣體相對於二氧化碳的暖化能力",
              "一種溫室氣體的排放量"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_347",
            "type": "考古題",
            "q": "什麼是碳邊境調整機制 (Carbon Border Adjustment Mechanism, CBAM)？",
            "options": [
              "僅使用環保材料的供應鏈",
              "對出口產品提供碳補貼的機制",
              "對高碳排放國家實施貿易制裁的機制",
              "以上皆非"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_377",
            "type": "考古題",
            "q": "在 LCIA 階段，最重要的指標之一為：",
            "options": [
              "碳足跡 (CFP)",
              "碳中和",
              "全球暖化潛勢 (GWP)",
              "碳稅"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_207",
            "type": "考古題",
            "q": "下列何者不是碳揭露計畫的揭露範疇領域？",
            "options": [
              "土壤",
              "氣候變遷",
              "水",
              "塑膠"
            ],
            "correct": 0,
            "explain": "CDP 揭露範疇為：氣候變遷、水安全、森林、塑膠（含生物多樣性",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_366",
            "type": "考古題",
            "q": "A 公司為實收資本額 60 億的上櫃公司。依金管會規定 A 公司必須完成盤查 並於民國哪一年開始申報？",
            "options": [
              "113 年",
              "114 年",
              "115 年",
              "116 年"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_341",
            "type": "考古題",
            "q": "什麼是氣候變遷風險評估？",
            "options": [
              "評估企業因氣候變遷導致財務損失的風險",
              "評估企業因碳排放超標被罰款的風險",
              "評估企業因投資綠能產業失敗的風險",
              "以上皆是"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_356",
            "type": "考古題",
            "q": "什麼是綠色交通(Green transportation)？",
            "options": [
              "使用電動車的交通",
              "使用公共交通的交通",
              "使用自行車或步行的交通",
              "以上皆是"
            ],
            "correct": 3,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_376",
            "type": "考古題",
            "q": "「CO₂ 當量 (CO₂e)」主要用於：",
            "options": [
              "統一不同氣體排放單位",
              "測量產品重量",
              "碳中和抵換額度",
              "溫度計算"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L112_Kogu_357",
            "type": "考古題",
            "q": "什麼是綠色金融 (Green Finance)？",
            "options": [
              "減少溫室氣體排放和增加碳匯的行動",
              "適應氣候變遷影響的行動",
              "預測未來氣候變化的科學研究",
              "促進國際合作應對氣候變遷的政策"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 5,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          }
        ]
      },
      {
        "id": "L121",
        "name": "ISO 14064-1 組織盤查",
        "subject": "淨零碳盤查規範與程序概要",
        "scope": "範圍、邊界、量化、清冊、報告書",
        "frameworkQuestion": "ISO 14064-1:2018 如何定義組織邊界？控制權法 vs 股權比例法差在哪？",
        "controversy": "為什麼 2018 版把「範疇一二三」改成「類別 1-6」？這對企業盤查實務有什麼影響？",
        "concepts": [
          { "id": "boundary", "name": "組織邊界與範疇", "tags": ["組織邊界","Scope","盤查","考古題"] },
          { "id": "emission-factor", "name": "排放係數與計算", "tags": ["排放係數","GWP","CO2e","直接監測","考古題"] },
          { "id": "report-principle", "name": "盤查報告原則", "tags": ["準確性","完整性","一致性","透明性","考古題"] }
        ],
        "frameworkQ": {
          "question": "組織型溫室氣體盤查的五大報告原則（準確性、完整性、一致性、透明性、相關性）各自防範什麼問題？",
          "hint": "想想看：如果一家企業只揭露 Scope 1 不揭露 Scope 2，違反了哪個原則？如果每年用不同計算方法，又是哪個？",
          "answer": "準確性：防止計算誤差和數據品質不足。完整性：防止漏報排放源（如只報 Scope 1 不報 Scope 2）。一致性：確保跨年可比較（不能每年換算法）。透明性：防止選擇性揭露。相關性：確保涵蓋所有重大排放源。"
        },
        "controversyQ": {
          "scenario": "排放係數法 vs 直接監測法，哪個才是「正確」的盤查方法？",
          "sideA": { "label": "排放係數法為主", "argument": "成本低、適用廣，政府公告排放係數（如電力係數 0.495 kgCO2e/kWh）即可計算。中小企業不可能每個排放源都裝監測設備。ISO 14064-1 也允許使用排放係數法。" },
          "sideB": { "label": "直接監測更準確", "argument": "排放係數是平均值，無法反映個別企業實際排放。電力係數每年公告一次，但實際用電碳強度隨時段和來源變化。高排放企業應投資連續排放監測系統（CEMS）。" },
          "insight": "實務做法是「分層管理」：重大排放源（佔總排 80%+）優先直接監測或使用供應商特定數據，次要排放源用排放係數法。ISO 14064-1 不指定方法，但要求說明方法選擇理由和不確定性。"
        },
        "questions": [
          {
            "id": "L121_Kogu_263",
            "type": "考古題",
            "q": "組織如何確保其溫室氣體盤查的一致性，以 項？",
            "options": [
              "每年更換計算方法",
              "維持使用相同的數據來源和計算方法",
              "僅報告最大的排放源",
              "忽略小規模排放源"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_266",
            "type": "考古題",
            "q": "企業進行溫室氣體盤查的第一步是什麼？",
            "options": [
              "設定邊界鑑別排放源",
              "收集活動數據與計算排放量",
              "請高階主管指派溫室氣體盤查小組",
              "忽略小規模排放源的影響"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_256",
            "type": "考古題",
            "q": "什麼是碳足跡標籤？",
            "options": [
              "標示產品碳排放量的標籤",
              "標示產品能源效率的標籤",
              "標示產品水足跡的標籤",
              "標示產品回收率的標籤"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_305",
            "type": "考古題",
            "q": "切斷準則（Cut-off Criteria）的主要目的為？",
            "options": [
              "節省計算時間，排除不重要的項目",
              "控制外部稽核",
              "用來定義碳稅率",
              "減少查驗要求"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_330",
            "type": "考古題",
            "q": "「從搖籃到工廠大門 (Cradle-to-Gate)」不包含哪一階段？",
            "options": [
              "原料開採",
              "製造加工",
              "產品運輸與使用",
              "能源消耗"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_275",
            "type": "考古題",
            "q": "直接監測法的特點是下列何者？",
            "options": [
              "成本低廉且易於實施",
              "準確度較高但少見",
              "常用於計算間接排放",
              "基於估算而非實際測量"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_323",
            "type": "考古題",
            "q": "碳足跡計算結果的可信度需透過？",
            "options": [
              "政府公告",
              "第三方查驗",
              "公司自評",
              "價格分析"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_302",
            "type": "考古題",
            "q": "碳足跡計算結果通常以哪種單位表示？",
            "options": [
              "公噸 CH₄",
              "公噸 N₂O",
              "公噸 CO₂ 當量 (CO₂e)",
              "碳稅額度"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_293",
            "type": "考古題",
            "q": "根據台灣環保署碳足跡計算指引，一級數據佔上游排放比需達多少%，包含 組織本身及供應商？",
            "options": [
              "5%",
              "10%",
              "15%",
              "20%"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_303",
            "type": "考古題",
            "q": "全球暖化潛勢 (GWP) 的數據主要由哪個機構發布？",
            "options": [
              "UNFCCC",
              "IPCC",
              "WTO",
              "ISO"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_329",
            "type": "考古題",
            "q": "「從搖籃到墳墓 (Cradle-to-Grave)」的碳盤查範圍，通常涵蓋：",
            "options": [
              "原料開採到產品出廠",
              "原料開採到使用後廢棄處理",
              "工廠大門到工廠大門",
              "產品使用階段"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_264",
            "type": "考古題",
            "q": "溫室氣體盤查報告原則中，何者是為了使溫室氣體相關資訊能有意義地比較？",
            "options": [
              "準確性",
              "一致性",
              "充分且適當地揭露溫室氣體相關資訊",
              "避免公開不確定性數據"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_265",
            "type": "考古題",
            "q": "依據標準，組織型溫室氣體盤查須納入所有相關排放源，此為哪項原則？",
            "options": [
              "準確性",
              "完整性",
              "僅報告國際間公認的溫室氣體",
              "避免使用地方性的溫室氣體數據"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_308",
            "type": "考古題",
            "q": "查驗 (Verification) 需依循哪個國際標準？",
            "options": [
              "ISO 14064-3",
              "ISO 9001",
              "ISO 26000",
              "ISO 50001"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_261",
            "type": "考古題",
            "q": "什麼是碳排放因子？",
            "options": [
              "計算溫室氣體排放量時使用的係數",
              "衡量溫室氣體排放強度的指標",
              "評估減碳潛力的工具",
              "以上皆非"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          }
        ],
        "reinforcement": [
          {
            "id": "L121_Kogu_284",
            "type": "考古題",
            "q": "提供服務時，應如何確保製程流程圖的完整",
            "options": [
              "假設所有服務流程相同",
              "僅記錄服務開始和結束",
              "思考被服務對象在系統中的全程體驗",
              "僅考慮服務的主要部分"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_271",
            "type": "考古題",
            "q": "溫室氣體量化方法中，排放係數法的計算公式，下列何者正確？",
            "options": [
              "CO2 排放當量=活動數據×排放係數×GWP",
              "CO2 排放當量=活動數據×人口數量×GWP",
              "CO2 排放當量=活動數據×面積占比×GWP",
              "CO2 排放當量=活動數據×重量占比×GWP"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_337",
            "type": "考古題",
            "q": "若企業僅盤查工廠內部製程排放，適用的範圍界定是：",
            "options": [
              "Cradle-to-Grave",
              "Gate-to-Gate",
              "Cradle-to-Gate",
              "Upstream-Downstream"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_257",
            "type": "考古題",
            "q": "什麼是能源標籤？",
            "options": [
              "標示產品碳排放量的標籤",
              "標示產品能源效率的標籤",
              "標示產品水足跡的標籤",
              "測量二氧化碳排放的工具"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_301",
            "type": "考古題",
            "q": "盤查資料收集需包含下列哪一項？",
            "options": [
              "產品標價",
              "製程流程圖",
              "競爭對手報告",
              "顧客滿意度"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_249",
            "type": "考古題",
            "q": "根據 ISO 14064-1 標準，企業在進行碳排放盤查時，應納入哪一範圍的排放？",
            "options": [
              "只包括直接排放",
              "包括直接和間接排放",
              "只包括間接排放",
              "只包括生產過"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_336",
            "type": "考古題",
            "q": "盤查範圍「Cradle-to-Grave」與「Cradle-to-Gate」的主要差異為：",
            "options": [
              "是否含製造階段",
              "是否含使用與廢棄階段",
              "是否含能源消耗",
              "是否含包裝階段"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_268",
            "type": "考古題",
            "q": "C 公司在溫室氣體盤查報告書提到盤查的 項？",
            "options": [
              "組織邊界",
              "營運邊界",
              "股權持分法",
              "以上皆是"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_274",
            "type": "考古題",
            "q": "碳足跡量化資訊應於碳足跡研究報告中揭 露，以下何者可以不包含於報告之中？",
            "options": [
              "針對電力之處理",
              "選定的分配方法",
              "內外部議題",
              "僅關注能源消耗相關的排放"
            ],
            "correct": 3,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L121_Kogu_252",
            "type": "考古題",
            "q": "某工廠在一年內的能源使用情況如下： 使用電力 500,000 kWh（每 kWh 電力產生 0.527 公斤二氧化碳） 使用柴油 20,000 公升（每公升柴油產生 2.68 公斤二氧化碳） 使用天然氣 100,000 立方米（每立方米天然氣產生 1.85 公斤二氧化碳） 請計算該工廠一年內總共產生的二氧化碳排放量。",
            "options": [
              "371,350 公斤",
              "502,100 公斤",
              "413,000 公斤",
              "421,500 公斤"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 2,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          }
        ]
      },
      {
        "id": "L122",
        "name": "ISO 14067 產品碳足跡",
        "subject": "淨零碳盤查規範與程序概要",
        "scope": "LCA、PCR、製程地圖、系統邊界、活動數據、排放係數、碳足跡計算",
        "frameworkQuestion": "產品碳足跡計算的四大步驟（目標範疇、清冊分析、衝擊評估、結果詮釋）各自要做什麼？",
        "controversy": "同一產品不同 PCR 可能算出差很多的碳足跡，這是標準問題還是必然的？",
        "concepts": [
          { "id": "lca-stages", "name": "生命週期評估階段", "tags": ["LCA","目標範疇","清單分析","衝擊評估","考古題"] },
          { "id": "cfp-calc", "name": "碳足跡計算方法", "tags": ["碳足跡","功能單位","系統邊界","切斷準則","考古題"] },
          { "id": "verification", "name": "查驗與不確定性", "tags": ["查驗","敏感度分析","不確定性","PCR","EPD","考古題"] }
        ],
        "frameworkQ": {
          "question": "產品碳足跡的「搖籃到墳墓」和「搖籃到大門」差在哪？什麼時候用哪個？",
          "hint": "想想看：一瓶礦泉水的碳足跡，要不要算消費者喝完後的瓶子回收？B2B 零組件呢？",
          "answer": "搖籃到墳墓（Cradle-to-Grave）：涵蓋原料→製造→運輸→使用→廢棄全生命週期，適用消費品（如食品、飲料）。搖籃到大門（Cradle-to-Gate）：只算到工廠出貨，適用 B2B 中間材料（如鋼材、化學品），因為下游使用和廢棄由客戶計算。選擇依據是 PCR（產品類別規則）規定。"
        },
        "controversyQ": {
          "scenario": "產品碳足跡的「切斷準則」允許忽略佔比 <1% 的排放源，這合理嗎？",
          "sideA": { "label": "合理且必要", "argument": "100% 涵蓋不切實際，數據收集成本會超過效益。1% 切斷讓企業聚焦重大排放源。ISO 14067 和 PAS 2050 都允許合理切斷。" },
          "sideB": { "label": "可能造成漏洞", "argument": "如果有 20 個 <1% 的排放源，加總可能佔 10-15%。切斷準則可能被故意利用來隱藏排放。缺乏統一的切斷門檻定義，各企業標準不一。" },
          "insight": "切斷準則的關鍵在「加總上限」：多數標準要求所有被切斷的排放源加總不得超過總排放的 5%。這防止了「化整為零」的漏洞。查驗時需要揭露切斷項目清單和理由。"
        },
        "questions": [
          {
            "id": "L122_Kogu_324",
            "type": "考古題",
            "q": "CFP 報告主要用途之一是？",
            "options": [
              "行銷宣傳與綠色標籤",
              "增加產品價格",
              "訂定稅率",
              "減少員工通勤"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_296",
            "type": "考古題",
            "q": "【已刪除】105. 透過製程地圖的每個步驟，查看哪些環節的碳排放較高，我們稱此環節為什麼，依據此結果作為未來企業實行減排策略?",
            "options": [
              "關鍵係數",
              "重要碳排點",
              "關鍵熱點",
              "高碳排點"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_335",
            "type": "考古題",
            "q": "LCA 最後一個階段「解釋 (Interpretation)」主要目的：",
            "options": [
              "檢查數據一致性與合理性",
              "蒐集原料排放量",
              "確認 PCR 規範",
              "轉換為碳標籤"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_280",
            "type": "考古題",
            "q": "在考慮未來盤查複雜度時，應如何處理製程 但 B2B 產品不需被納入的生命週期階段 流程圖？",
            "options": [
              "完全不進行簡化",
              "隨意刪除一些看似不重要的步驟",
              "做適度簡化，保留重要信息",
              "僅保留開頭和結尾的步驟"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_286",
            "type": "考古題",
            "q": "衝擊評估階段屬於生命週期評估 (LCA) 作業的哪個階段？",
            "options": [
              "第一階段",
              "第二階段",
              "允許忽略對總排放貢獻微小的過程",
              "增加碳足跡計算的準確性"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_290",
            "type": "考古題",
            "q": "在生命週期評估中，目標與範疇界定的重要 性是什麼？",
            "options": [
              "決定產品的價格",
              "界定評估的範圍和目的，確保評估結果的有",
              "選擇適合的廣告平台",
              "計算生產成本"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_338",
            "type": "考古題",
            "q": "ISO 14067 要求的「代表性」指的是：",
            "options": [
              "與市場數據一致",
              "與時間、地理、技術條件相符",
              "與產品價格一致",
              "與法規一致"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_341",
            "type": "考古題",
            "q": "產品碳足跡計算時，GWP 值應依據：",
            "options": [
              "企業自訂係數",
              "ISO 14064-3",
              "IPCC 公布之 AR5 或 AR6",
              "當地法規允許的排放係數"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_311",
            "type": "考古題",
            "q": "CFP 中「敏感度分析」的主要用途是？",
            "options": [
              "評估特定假設改變對結果的影響",
              "替代 PCR",
              "調整切斷準則",
              "決定碳稅額度"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_289",
            "type": "考古題",
            "q": "使用生命週期評估進行產品碳足跡評估不",
            "options": [
              "盤查分析",
              "目標範疇定義",
              "計算產品的生產成本和價格",
              "確定產品的銷售地區和目標客戶"
            ],
            "correct": 3,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_278",
            "type": "考古題",
            "q": "產品碳足跡 (CFP) 量化時，為何需要考慮產品的整個生命週期？",
            "options": [
              "降低成本",
              "僅計算一次相同的 GHG 排放與移除",
              "功能單位或宣告單位",
              "僅關注直接排放"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_326",
            "type": "考古題",
            "q": "CFP 中常用於數據不確定性檢驗的方法是？",
            "options": [
              "敏感度分析",
              "SWOT",
              "財報分析",
              "回歸模型"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_309",
            "type": "考古題",
            "q": "LCA 的衝擊評估階段主要用於？",
            "options": [
              "確定碳稅額度",
              "評估不同排放對環境影響",
              "決定 PCR 類別",
              "編製財報"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_315",
            "type": "考古題",
            "q": "功能單位 (Functional Unit) 的作用是？",
            "options": [
              "表示碳稅額度",
              "提供比較基準",
              "測量產品價格",
              "確認 PCR 類別"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_307",
            "type": "考古題",
            "q": "CFP 報告書需遵循哪一項原則？",
            "options": [
              "ISO 50001 能源管理原則",
              "ISO 14067 報告規範",
              "WTO 環境貿易規範",
              "COP 議定書"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          }
        ],
        "reinforcement": [
          {
            "id": "L122_Kogu_321",
            "type": "考古題",
            "q": "CFP 數據收集原則應遵循？",
            "options": [
              "透明性、完整性、準確性、可比性",
              "價格導向",
              "競爭者參考",
              "市場占有率"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_260",
            "type": "考古題",
            "q": "什麼是碳足跡管理？",
            "options": [
              "減少產品碳排放量的過程",
              "抵換產品碳排放量的過程",
              "規劃、執行、監測和報告產品碳足跡的過程",
              "以上皆是"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_291",
            "type": "考古題",
            "q": "生命週期衝擊評估(LCIA)的主要目標是什 麼？",
            "options": [
              "確定產品的最終銷售價格",
              "評估產品生命週期中的環境衝擊",
              "分析公司的財務狀況",
              "評估市場競爭情況"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_328",
            "type": "考古題",
            "q": "CFP 計算的最終目的為？",
            "options": [
              "提升企業永續管理與供應鏈透明度",
              "增加碳稅負擔",
              "僅符合法規要求",
              "減少審查成本"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_262",
            "type": "考古題",
            "q": "什麼是產品碳足跡？",
            "options": [
              "產品在生產過程中產生的碳排放量",
              "產品在使用過程中產生的碳排放量",
              "產品在廢棄處理過程中產生的碳排放量",
              "產品從原料取得、製造、配送、使用到廢棄"
            ],
            "correct": 3,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_306",
            "type": "考古題",
            "q": "不確定性分析的主要目的為何？",
            "options": [
              "減少碳稅負擔",
              "確保計算結果具合理可信度",
              "增加PCR適用性",
              "取代敏感度分析"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_279",
            "type": "考古題",
            "q": "產品碳足跡過程盤查中，繪製製程地圖是一",
            "options": [
              "列出流程各關卡負責單位與人員",
              "列出流程內的投入與產出",
              "確定繪製標的及其功能/宣告單位",
              "列出製造產品，須經過哪些步驟"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_318",
            "type": "考古題",
            "q": "LCA 衝擊評估 (LCIA) 的作用是？",
            "options": [
              "評估不同氣體對暖化的貢獻",
              "計算產品成本",
              "制定 PCR",
              "編製碳費收據"
            ],
            "correct": 0,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_294",
            "type": "考古題",
            "q": "透過製程地圖的每個步驟，查看哪些環節的碳排放較高，我們稱此環節為什 麼，依據此結果作為未來企業實行減排策略?",
            "options": [
              "關鍵係數",
              "重要碳排點",
              "關鍵熱點",
              "高碳排點"
            ],
            "correct": 2,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          },
          {
            "id": "L122_Kogu_342",
            "type": "考古題",
            "q": "LCA 的「功能單位」主要用於：",
            "options": [
              "衡量碳稅金額",
              "產品間的公平比較基準",
              "盤查報告書的頁數",
              "溫室氣體抵換額度"
            ],
            "correct": 1,
            "explain": "（考古題，官方答案）",
            "epRef": 38,
            "source": "kogu",
            "tag": [
              "考古題"
            ]
          }
        ]
      }
    ]
  },
  "l2": {
    "name": "中級",
    "nameFull": "iPAS 淨零碳規劃管理師（中級）",
    "examFormat": "單選 + 複選 + 情境題 × 90 分鐘 × 2 科，70 分及格",
    "focus": "決策與計算:情境、財務、判斷",
    "examStyle": "🎯 2026/04/11 首場考試特徵：題幹變短、跨章節混合題多、「半對半錯」選項陷阱、實務情境判斷為主。準備方向是「顧問思維」，用情境練習而非單純背誦。",
    "modules": [
      {
        "id": "L211",
        "name": "節能減碳策略",
        "subject": "節能減碳技術實務",
        "scope": "目標設定、碳排熱區辨識、NPV/IRR/回收期、財務決策",
        "examTips": "🔥 4/11 實考重點：NPV 投資決策計算題、BAU 基準設定、絕對/強度目標陷阱（半對半錯）、KPI 管理機制設計",
        "concepts": [
          { "id": "finance", "name": "財務決策與投資分析", "tags": ["NPV","IRR","回收期","財務決策","MACC","ESCO"] },
          { "id": "target", "name": "目標設定與基準線", "tags": ["BAU","SBTi","KPI","自主減量"] },
          { "id": "carbon-cost", "name": "碳成本與內部定價", "tags": ["碳費","ICP","EnPI"] },
          { "id": "strategy", "name": "減碳策略整合", "tags": ["ISO50001","整合","管理"] }
        ],
        "frameworkQuestion": "企業要設「絕對減量」還是「強度減量」目標？兩者對成長型產業的影響差異？",
        "controversy": "NPV 正的節能案就該投嗎？回收期 5 年跟 IRR 12% 哪個判斷標準更好？",
        "frameworkQ": {
          "question": "企業要設「絕對減量」還是「強度減量」目標？兩者對成長型產業的影響差異？",
          "hint": "想想看：如果工廠產能翻倍，哪種目標會讓你的減碳承諾變得不可能達成？",
          "answer": "絕對減量（如 SBTi 要求）設定總排放上限，產能成長時壓力極大；強度減量（如每噸產品排放）允許總量隨產能成長，但可能「越減越多」。多數國際框架要求絕對目標，但允許設定強度中間目標作為過渡。關鍵判斷：你的產業是否在成長期？成長幅度是否超過減碳技術的改善速度？"
        },
        "controversyQ": {
          "scenario": "NPV 正的節能案就該投嗎？回收期 5 年跟 IRR 12% 哪個判斷標準更好？",
          "sideA": { "label": "回收期優先", "argument": "回收期直覺易懂，決策快。中小企業現金流緊，3-5 年回收是實務門檻。碳費初期低（$300/噸），財務效益有限，長期 NPV 不確定性高。" },
          "sideB": { "label": "NPV/IRR 優先", "argument": "回收期忽略時間價值和碳費逐年遞增效果。IRR 12% 可能低估 SBTi 合規壓力帶來的隱性收益。碳費政策確定上漲，越早投入 NPV 優勢越大。" },
          "insight": "實務上三個指標要一起看：回收期篩初選（排除 >7 年）、IRR 排序優先級、NPV 納入碳費遞增和 ICP 情境做最終決策。單用任何一個都會漏判。"
        },
        "questions": [
          {
            "id": "L211_Q01",
            "type": "計算",
            "q": "空壓機汰換案：初始投資 800 萬，未來 6 年每年節能效益 200 萬，折現率 10%。NPV 約為？",
            "options": [
              "約 +400 萬",
              "約 +71 萬，可投資",
              "約 -29 萬，不應投資（折現率誤用 12%）",
              "約 +1,200 萬（年金未扣初投）"
            ],
            "correct": 1,
            "explain": "AF(10%,6)=3.9927...實際 4.3553；200×4.3553=870.6 萬；NPV=870.6-800=70.6≈71 萬。A 未折現；C 用錯折現率；D 漏扣初投。",
            "epRef": 4,
            "tag": [
              "NPV",
              "計算"
            ]
          },
          {
            "id": "L211_Q02",
            "type": "計算",
            "q": "LED 改造案：初始 1,200 萬，年節省穩定 300 萬（不考慮折現），簡單回收期為？",
            "options": [
              "3 年（漏計維護費直覺誤判）",
              "4 年",
              "5 年（用折現回收期概念但算錯）",
              "6 年（以總效益期反推）"
            ],
            "correct": 1,
            "explain": "簡單回收期=初投/年節省=1200/300=4 年。題幹明確不考慮折現，故非折現回收期。",
            "epRef": 4,
            "tag": [
              "回收期",
              "計算"
            ]
          },
          {
            "id": "L211_Q03",
            "type": "計算",
            "q": "某節能案初始投資 600 萬，第 5 年末一次性回收 900 萬（期間無現金流），IRR 約為？",
            "options": [
              "約 6.5%（以平均年報酬估算）",
              "約 8.45%",
              "約 10%",
              "約 50%（總報酬率當 IRR）"
            ],
            "correct": 1,
            "explain": "600=900/(1+r)^5 → (1+r)^5=1.5 → r=1.5^0.2-1≈8.45%。C 混淆總報酬與年化；D 未年化。",
            "epRef": 4,
            "tag": [
              "IRR",
              "計算"
            ]
          },
          {
            "id": "L211_Q04",
            "type": "計算",
            "q": "熱回收投資 500 萬，未來 5 年節省 120/180/220/180/120 萬，折現率 8%，NPV 最接近？",
            "options": [
              "約 +820 萬（未扣初投）",
              "約 +154 萬",
              "約 -154 萬（符號反向）",
              "約 +220 萬（取最大年效益外推）"
            ],
            "correct": 1,
            "explain": "逐年折現加總≈654.1 萬，NPV=654.1-500≈154 萬。A 未扣初投；C 符號錯。",
            "epRef": 4,
            "tag": [
              "NPV",
              "計算"
            ]
          },
          {
            "id": "L211_Q05",
            "type": "單選",
            "q": "關於能源績效指標（EnPI）與基準（EnB）的敘述，何者正確？",
            "options": [
              "EnPI 必須以絕對用電量表示，且不得隨產量調整",
              "EnPI 應依使用情境標準化（如每單位產品耗能），EnB 需在條件改變時重新建立",
              "EnB 一旦建立後即為永久基準，完全不可更動",
              "只要 EnPI 下降就代表節能有效，無需考量產量變化，因為指標本身已排除產量影響因素"
            ],
            "correct": 1,
            "explain": "B 符合 ISO 50001。A「不得隨產量調整」錯；C「完全不可更動」絕對語陷阱；D 忽略產量變化是半對半錯經典。",
            "epRef": 4,
            "tag": [
              "EnPI",
              "陷阱",
              "絕對語"
            ]
          },
          {
            "id": "L211_Q06",
            "type": "情境",
            "q": "工廠年排 2 萬噸 CO2e，主管欲設 KPI 驅動減碳。下列 KPI 設計最符合實務管理精神？",
            "options": [
              "僅設定總排放量絕對減量目標，不分解至製程或部門別指標",
              "設定總量目標，並拆解至各製程強度指標，搭配月度檢討",
              "完全以碳費支出作為唯一 KPI，排放量次要",
              "KPI 僅由高階主管掌握，產線人員無須知悉"
            ],
            "correct": 1,
            "explain": "B 結合總量+強度+定期檢討為最佳實務。A 缺乏拆解；C「唯一」絕對語；D 違背全員參與原則。",
            "epRef": 4,
            "tag": [
              "KPI",
              "情境"
            ]
          },
          {
            "id": "L211_Q07",
            "type": "單選",
            "q": "關於 BAU（Business As Usual）基準情境的設定，何者正確？",
            "options": [
              "BAU 應反映若不採行減碳措施的預期排放軌跡，含產能擴張與能耗趨勢",
              "BAU 必須等於基準年靜態排放，完全不得外推",
              "BAU 設定後即鎖定 10 年不可修正，以維持基準線一致性並避免人為操縱",
              "BAU 只適用於國家層級，企業層級不得使用"
            ],
            "correct": 0,
            "explain": "A 正確。B「完全不得外推」、C「絕對不可修正」、D「不得使用」皆為絕對語陷阱。",
            "epRef": 4,
            "tag": [
              "BAU",
              "陷阱",
              "絕對語"
            ]
          },
          {
            "id": "L211_Q08",
            "type": "情境",
            "q": "某廠考慮汰換老舊鍋爐，初估 NPV 為正但邊際（+30 萬），主管應如何決策最符合策略？",
            "options": [
              "NPV 為正就立即投資，不需考慮碳費變動、設備殘值或政策風險等因素",
              "檢視與 SBTi 減碳路徑、碳費曝險、設備殘值風險後再定",
              "一律否決，因 NPV 太低",
              "以 IRR 單一指標決定，忽略 NPV"
            ],
            "correct": 1,
            "explain": "邊際案件應納入策略面（SBTi 路徑、碳費風險）綜合判斷。A 忽略風險；C 僵化；D 單一指標偏誤。",
            "epRef": 4,
            "tag": [
              "財務決策",
              "情境",
              "跨章"
            ]
          },
          {
            "id": "L211_Q09",
            "type": "整合",
            "q": "A 公司年排 3 萬噸擬提自主減量計畫（優惠費率 50/噸 vs 一般 300/噸），同時評估 2,000 萬投資可年減 5,000 噸。該投資評估 NPV 時年效益應含？",
            "options": [
              "僅計節能電費，不計碳費扣減",
              "節能電費 + 碳費減少（以優惠費率 50/噸計年減 5,000 噸=25 萬）",
              "節能電費 + 碳費減少（以一般費率 300/噸 × 年減 5,000 噸 = 150 萬計入）",
              "僅計碳費扣減，不計電費"
            ],
            "correct": 1,
            "explain": "若適用自主減量計畫優惠費率，碳費扣減應以 50/噸計，否則高估效益。C 是常見陷阱（用一般費率）。",
            "epRef": 4,
            "tag": [
              "跨章",
              "NPV",
              "碳費"
            ]
          },
          {
            "id": "L211_Q10",
            "type": "整合",
            "q": "企業採用 ICP（內部碳價）500 元/噸輔助投資決策，某案年減碳 2,000 噸、年節電 100 萬、初投 800 萬、年限 10 年、折現率 8%。年總效益應為？",
            "options": [
              "100 萬（僅計電費）",
              "200 萬（100 萬電費 + 2000×500=100 萬 ICP 影子收益）",
              "約 700 萬",
              "60 萬（僅計碳費 300×2,000 噸，不計 ICP 影子收益與節能電費減免效益）"
            ],
            "correct": 1,
            "explain": "ICP 為影子價格，評估時以 ICP 而非實收碳費計算，強化長期減碳案競爭力。",
            "epRef": 4,
            "tag": [
              "ICP",
              "整合",
              "跨章"
            ]
          },
          {
            "id": "L211_Q11",
            "type": "單選",
            "q": "關於能源審查（Energy Review）的敘述：",
            "options": [
              "應識別重大能源使用（SEU），並排序節能機會，但不必每年更新",
              "應識別 SEU 並依使用情境排序節能機會，定期審查並更新",
              "只需盤點總用電，無須分製程",
              "一次審查後永久有效，完全不必複審"
            ],
            "correct": 1,
            "explain": "B 符合 ISO 50001。A「不必每年更新」半對半錯；C 缺分析；D「永久有效」絕對語。",
            "epRef": 4,
            "tag": [
              "ISO50001",
              "陷阱"
            ]
          },
          {
            "id": "L211_Q12",
            "type": "情境",
            "q": "廠區進行節能改善，下列何者屬於「管理面」而非「技術面」改善？",
            "options": [
              "更換高效變頻馬達",
              "導入能源監控系統並建立月度檢討機制",
              "加裝熱回收設備",
              "更換 LED 照明即可達成全部節能目標"
            ],
            "correct": 1,
            "explain": "管理面強調制度、監控、檢討；A/C/D 皆屬技術設備面。",
            "epRef": 4,
            "tag": [
              "管理",
              "情境"
            ]
          },
          {
            "id": "L211_Q13",
            "type": "單選",
            "q": "關於淨現值（NPV）與內部報酬率（IRR）：",
            "options": [
              "NPV 為正則可投資，IRR 大於資金成本亦可，但兩者互斥方案時以 IRR 為準",
              "NPV 為正且 IRR 大於資金成本通常可投資；互斥方案應以 NPV 為準",
              "IRR 永遠優於 NPV，應僅用 IRR",
              "NPV 計算時不必考慮折現率"
            ],
            "correct": 1,
            "explain": "互斥方案以 NPV 為準（規模效應）。A 後半錯；C「永遠優於」絕對語；D 錯誤。",
            "epRef": 4,
            "tag": [
              "NPV",
              "IRR",
              "陷阱"
            ]
          },
          {
            "id": "L211_Q14",
            "type": "情境",
            "q": "MACC（邊際減量成本曲線）最適用情境為？",
            "options": [
              "單一設備採購決策",
              "企業盤點多項減碳措施並排序投資順序",
              "替代碳足跡計算",
              "取代 LCA 分析作為企業碳管理唯一工具"
            ],
            "correct": 1,
            "explain": "MACC 列出各措施單位減碳成本並排序，適用整體減碳投資組合規劃。",
            "epRef": 4,
            "tag": [
              "MACC",
              "情境"
            ]
          },
          {
            "id": "L211_Q15",
            "type": "單選",
            "q": "關於 ESCO（能源技術服務業）合約：",
            "options": [
              "保證節能績效型（Guaranteed Savings）由業主融資，ESCO 保證節能量",
              "ESCO 永遠承擔全部財務風險",
              "分享節能績效（Shared Savings）由業主全額融資並承擔技術風險，ESCO 僅負責分潤",
              "ESCO 合約完全不需要 M&V（量測驗證）"
            ],
            "correct": 0,
            "explain": "A 正確。B「永遠承擔全部」絕對語；C 定義顛倒（Shared 由 ESCO 融資）；D「完全不需 M&V」錯。",
            "epRef": 4,
            "tag": [
              "ESCO",
              "陷阱"
            ]
          }
        ],
        "reinforcement": [
          {
            "id": "L211_R01",
            "type": "計算",
            "q": "投資 1,000 萬，每年現金流 250 萬連續 5 年，折現率 8%，NPV 約為？",
            "options": [
              "約 +250 萬",
              "約 -2 萬（邊際）",
              "約 +998 萬（漏扣初投）",
              "約 +1,250 萬"
            ],
            "correct": 1,
            "explain": "AF(8%,5)=3.9927，250×3.9927=998.2，NPV≈-1.8 萬，邊際案件。",
            "epRef": 4,
            "tag": [
              "NPV",
              "加強"
            ]
          },
          {
            "id": "L211_R02",
            "type": "情境",
            "q": "提交自主減量計畫時，下列何者為必要要件？",
            "options": [
              "承諾達特定減量率（目前 SBTi 或國家路徑）並附查驗",
              "只要提交自主減量計畫文件即可獲優惠費率，無需達成實質減量目標",
              "僅需口頭承諾",
              "無需第三方查驗"
            ],
            "correct": 0,
            "explain": "自主減量計畫需達指定減量率並經查驗，始適用優惠費率。",
            "epRef": 4,
            "tag": [
              "自主減量",
              "跨章"
            ]
          },
          {
            "id": "L211_R03",
            "type": "單選",
            "q": "關於折現回收期（Discounted Payback）：",
            "options": [
              "以折現後現金流累積達初投之年數，通常長於簡單回收期",
              "永遠等於簡單回收期",
              "僅適用於單次現金流",
              "與 NPV 計算無關，僅用於會計帳面折舊處理而非投資決策"
            ],
            "correct": 0,
            "explain": "折現回收期納入時間價值，故通常長於簡單回收期。",
            "epRef": 4,
            "tag": [
              "回收期",
              "加強"
            ]
          },
          {
            "id": "L211_R04",
            "type": "整合",
            "q": "某企業年排 3 萬噸直接排放，已承諾 SBTi 短期目標路徑，面臨碳費徵收，評估節能投資方案時應如何決策？",
            "options": [
              "只看 NPV 是否為正即可決策，不需對齊 SBTi 路徑或考量碳費遞增",
              "納入逐年碳費遞增、SBTi 路徑合規性、ICP 情境分析",
              "只看碳費省下金額",
              "忽略 SBTi，法規未強制"
            ],
            "correct": 1,
            "explain": "策略性投資需綜合財務、法規曝險、科學目標路徑。",
            "epRef": 4,
            "tag": [
              "跨章",
              "SBTi",
              "加強"
            ]
          },
          {
            "id": "L211_R05",
            "type": "情境",
            "q": "實施能源管理系統後，某廠 EnPI 下降 5% 但總用電上升 10%，最可能原因？",
            "options": [
              "節能無效",
              "產量成長大於節能效果（強度下降但總量上升）",
              "計算錯誤",
              "EnPI 定義錯誤導致指標失真，與實際耗能無關"
            ],
            "correct": 1,
            "explain": "EnPI 為強度指標，產量成長可致總量上升但強度下降，顯示節能有效但需額外管理。",
            "epRef": 4,
            "tag": [
              "EnPI",
              "加強"
            ]
          }
        ]
      },
      {
        "id": "L212",
        "name": "節能技術與能源管理",
        "subject": "節能減碳技術實務",
        "scope": "公用設施、ESCO、ISO 50001、氫能、儲能、智慧能源",
        "examTips": "🔥 4/11 實考重點：空調變頻節能實務題、ISO 50001 能源績效指標、技術「適用性」判斷（不考原理）",
        "concepts": [
          { "id": "iso50001", "name": "ISO 50001 能源管理", "tags": ["ISO50001","EnPI","EnB","SEU","M&V"] },
          { "id": "esco", "name": "ESCO 節能服務", "tags": ["ESCO"] },
          { "id": "equipment", "name": "設備節能技術", "tags": ["空調","變頻","馬達","LED","照明"] },
          { "id": "future-energy", "name": "前瞻能源技術", "tags": ["氫能","儲能","V2G","需量反應","DR","AMI","EMS","智慧能源"] }
        ],
        "frameworkQuestion": "ESCO 保證節能績效合約（Shared Saving vs Guaranteed Saving）的風險分攤怎麼算？",
        "controversy": "綠氫、藍氫、灰氫的真實碳排差多少？國家推氫能政策是科學還是產業補貼？",
        "frameworkQ": {
          "question": "ESCO 保證節能績效合約（Shared Saving vs Guaranteed Saving）的風險分攤怎麼算？",
          "hint": "想想看：如果節能效果比預期差 30%，兩種合約模式下誰要承擔損失？",
          "answer": "Shared Saving：ESCO 出資、承擔技術風險，業主分享節能效益（通常 60:40 到 80:20）。Guaranteed Saving：業主融資、ESCO 保證節能量，未達標 ESCO 賠差額。前者適合缺資金的企業，後者適合有資金但缺技術的企業。關鍵差異在「誰出錢」和「誰保證」。"
        },
        "controversyQ": {
          "scenario": "綠氫、藍氫、灰氫的真實碳排差多少？國家推氫能政策是科學還是產業補貼？",
          "sideA": { "label": "氫能是減碳關鍵", "argument": "綠氫生命週期近零碳排，可解決工業高溫製程（鋼鐵、水泥）無法電氣化的困境。藍氫搭配 CCUS 是過渡方案，碳排可降 85-95%。各國投入兆元級預算不是盲目補貼。" },
          "sideB": { "label": "氫能被過度炒作", "argument": "綠氫成本是灰氫 3-5 倍，2030 前難具競爭力。藍氫的 CCUS 捕獲率實際只有 60-80%，遠低於宣稱。電解水效率 60-70%，不如直接用電。許多氫能項目是化石燃料產業的漂綠工具。" },
          "insight": "氫能有明確場景（工業高溫、長途運輸、儲能），但不是萬靈丹。評估時看「邊際減碳成本」：同一筆錢用在直接電氣化 vs 氫能，哪個減碳效益高？場景對了才值得投入。"
        },
        "questions": [
      {
            "id": "L212_Q01",
            "type": "情境",
            "q": "某紡織廠評估導入節能改善，公司資金充裕但缺乏專業技術與節能量評估能力，希望由外部廠商承諾節能績效，若達不到目標由對方補足差額。此情境最適合採用何種 ESCO 合約模式？",
            "options": [
                  "節能保證型（Guaranteed Saving）",
                  "節能分享型（Shared Saving），由業主全額出資",
                  "能源費用包乾型（Chauffage）",
                  "BOT 委外營運模式"
            ],
            "correct": 0,
            "explain": "業主出資、廠商保證節能量並承擔績效風險，屬於 Guaranteed Saving。Shared Saving 則由 ESCO 出資。",
            "epRef": 11,
            "tag": [
                  "ESCO"
            ]
      },
      {
            "id": "L212_Q02",
            "type": "情境",
            "q": "某中小企業欲改善照明與空壓系統但無力投入改善資金，希望由能源技術服務業者出資施作，日後以節能效益分期回收。下列何種合約模式最符合需求？",
            "options": [
                  "節能保證型",
                  "節能分享型",
                  "自行投資模式",
                  "統包工程合約"
            ],
            "correct": 1,
            "explain": "Shared Saving 由 ESCO 出資承擔投資風險，節能效益依比例分享予雙方，適合資金不足但想改善的業主。",
            "epRef": 11,
            "tag": [
                  "ESCO"
            ]
      },
      {
            "id": "L212_Q03",
            "type": "判斷",
            "q": "關於 ESCO 專案簽訂前的前置作業，下列敘述何者正確？",
            "options": [
                  "可直接依廠商報價簽約，毋需進行現場量測",
                  "應先完成能源查核與基線量測，作為節能績效驗證依據",
                  "僅需提供電費單即可由 ESCO 估算節能潛力並簽約",
                  "節能基準通常以簽約後首月用電量為準"
            ],
            "correct": 1,
            "explain": "簽約前需建立能源基線（EnB）作為 M&V 節能量驗證依據，否則無法客觀認定節能績效。",
            "epRef": 11,
            "tag": [
                  "ESCO",
                  "M&V"
            ]
      },
      {
            "id": "L212_Q04",
            "type": "ISO50001",
            "q": "某食品廠依 ISO 50001 建立能源績效指標，發現夏季冷凍庫耗電顯著高於冬季，在設定 EnPI 時應如何處理此變異？",
            "options": [
                  "忽略季節差異，直接以年度總用電為 EnPI",
                  "將外氣溫度納入修正因子進行標準化",
                  "以最高耗電月作為基準期代表值",
                  "改用產值（元）作為唯一分母即可消除溫度影響"
            ],
            "correct": 1,
            "explain": "氣候為典型修正因子，需納入迴歸模型以消除環境變數對能源績效的干擾，才能反映真實改善成效。",
            "epRef": 11,
            "tag": [
                  "ISO50001",
                  "EnPI"
            ]
      },
      {
            "id": "L212_Q05",
            "type": "ISO50001",
            "q": "在 ISO 50001 能源審查中辨識「重大能源使用（SEU）」之目的，下列何者最為正確？",
            "options": [
                  "找出耗能占比高或改善潛力大的設備以優先管理",
                  "列出廠內所有設備做為盤查清單，不論耗能占比高低均同等管理",
                  "決定再生能源採購比例",
                  "作為碳權交易的抵換依據"
            ],
            "correct": 0,
            "explain": "SEU 聚焦於耗能占比大或具顯著改善潛力者，便於資源集中投入；非全廠清單，也與碳權交易無直接關聯。",
            "epRef": 11,
            "tag": [
                  "ISO50001",
                  "SEU"
            ]
      },
      {
            "id": "L212_Q06",
            "type": "ISO50001",
            "q": "某工廠近三年產量波動大，欲選擇能源基準期（EnB），下列選擇原則何者最適切？",
            "options": [
                  "選擇產量最低的一年，避免基線過高",
                  "選擇營運狀態具代表性且資料完整之期間",
                  "以上一個會計年度為固定基準，不可調整",
                  "選擇設備大修後第一個月"
            ],
            "correct": 1,
            "explain": "基準期應能代表正常營運條件並具完整可驗證資料；當結構性變化發生時應重新設定，而非挑最有利的一年。",
            "epRef": 11,
            "tag": [
                  "ISO50001",
                  "EnB"
            ]
      },
      {
            "id": "L212_Q07",
            "type": "氫能",
            "q": "某化工廠宣稱其產線使用「低碳氫」，來源為天然氣重組後搭配二氧化碳捕捉封存。此氫氣應歸類為何種類別？",
            "options": [
                  "綠氫",
                  "藍氫",
                  "灰氫",
                  "粉紅氫"
            ],
            "correct": 1,
            "explain": "天然氣重組（SMR）產氫並搭配 CCUS 捕捉減少碳排者為藍氫；若無 CCUS 即為灰氫，再生能源電解才是綠氫。",
            "epRef": 11,
            "tag": [
                  "氫能"
            ]
      },
      {
            "id": "L212_Q08",
            "type": "氫能",
            "q": "關於各類氫能的敘述，下列何者正確？",
            "options": [
                  "粉紅氫以燃煤電力電解水製成，碳強度最低且成本最便宜",
                  "黃氫碳強度固定為零，因其使用電網電力",
                  "綠氫由再生能源電解水產生，生命週期碳排接近零",
                  "灰氫搭配碳捕捉後即成為綠氫"
            ],
            "correct": 2,
            "explain": "綠氫為再生能源電解，碳排最低；粉紅氫為核能電解；黃氫碳排依電網組合而定；灰氫加 CCUS 為藍氫而非綠氫。",
            "epRef": 11,
            "tag": [
                  "氫能"
            ]
      },
      {
            "id": "L212_Q09",
            "type": "情境",
            "q": "某辦公大樓空調系統原採定頻冰水主機，負載在尖峰與離峰差異大，經常於部分負載運轉。下列何項改善策略的節能效益通常最顯著？",
            "options": [
                  "將冰水主機更換為更大噸數機型以備不時之需",
                  "導入變頻驅動並配合負載調整轉速",
                  "延長設備運轉時間以避免頻繁啟停",
                  "提高冰水出水溫度設定以減少冷卻負擔，同時關閉外氣引入"
            ],
            "correct": 1,
            "explain": "部分負載情境下變頻控制可依實際需求調整轉速，節能效益顯著；選項D後半關閉外氣會影響室內空氣品質。",
            "epRef": 11,
            "tag": [
                  "空調",
                  "變頻"
            ]
      },
      {
            "id": "L212_Q10",
            "type": "情境",
            "q": "某工廠欲更換老舊馬達以提升能源效率，下列選用原則何者最為適切？",
            "options": [
                  "一律選最大功率以保留裕度",
                  "依實際負載選擇高效率等級（如 IE3 以上）並避免長期輕載運轉",
                  "選購二手馬達以降低初始成本",
                  "所有設備一律改為直流馬達即可達最佳節能效果，無需依據負載特性選型"
            ],
            "correct": 1,
            "explain": "應依實際負載匹配功率與高效率等級；馬達過大導致長期輕載會降低效率，並非功率越大越好。",
            "epRef": 11,
            "tag": [
                  "馬達"
            ]
      },
      {
            "id": "L212_Q11",
            "type": "情境",
            "q": "某物流倉庫原使用 T8 螢光燈，照明時間長且維護成本高。評估導入 LED 照明時，下列考量何者較不適切？",
            "options": [
                  "比較生命週期成本而非僅初始購置費",
                  "考量色溫、照度是否符合作業需求",
                  "僅以單價最低者為採購標準",
                  "評估燈具配光與既有吊掛間距相容性"
            ],
            "correct": 2,
            "explain": "LED 雖單價高但壽命與效率優勢需以生命週期成本評估；僅看單價易忽略長期營運效益。",
            "epRef": 11,
            "tag": [
                  "LED",
                  "照明"
            ]
      },
      {
            "id": "L212_Q12",
            "type": "儲能",
            "q": "某離島微電網搭配再生能源，需於無風無光時段提供長達 6-10 小時穩定供電，且要求可循環壽命長。下列儲能技術何者最為適合？",
            "options": [
                  "飛輪儲能",
                  "鋰離子電池",
                  "液流電池",
                  "超級電容"
            ],
            "correct": 2,
            "explain": "液流電池功率與容量可獨立設計、循環壽命長，適合長時儲能；飛輪與超電容為短時高功率，鋰電池長時成本偏高。",
            "epRef": 11,
            "tag": [
                  "儲能"
            ]
      },
      {
            "id": "L212_Q13",
            "type": "儲能",
            "q": "某電網需於秒級內平衡頻率波動，強調高功率快速響應而非長時間放電。下列何種儲能最符合此需求？",
            "options": [
                  "抽蓄水力",
                  "壓縮空氣儲能",
                  "飛輪儲能",
                  "液流電池"
            ],
            "correct": 2,
            "explain": "飛輪響應速度極快、適合短時高功率調頻；抽蓄與 CAES 屬大型長時儲能，液流電池適合長時穩定放電。",
            "epRef": 11,
            "tag": [
                  "儲能"
            ]
      },
      {
            "id": "L212_Q14",
            "type": "智慧能源",
            "q": "某百貨業者與電力公司簽訂需量反應（DR）契約，下列敘述何者正確？",
            "options": [
                  "業者須於系統尖峰時依通知抑低用電，可獲電費回饋",
                  "需量反應等同於完全停電，業者須關閉所有設備",
                  "參與 DR 後業者失去自主用電權，由台電遠端控制所有負載",
                  "DR 僅適用於住宅用戶，工業用戶不可參與"
            ],
            "correct": 0,
            "explain": "需量反應為業者依通知主動降載以換取電費回饋，非強制停電；工業與商業用戶為主要參與者。",
            "epRef": 11,
            "tag": [
                  "需量反應",
                  "DR"
            ]
      },
      {
            "id": "L212_Q15",
            "type": "智慧能源",
            "q": "某工業園區導入能源管理系統（EMS）搭配智慧電表（AMI），下列應用何者最為典型？",
            "options": [
                  "即時監測各區耗能並辨識異常，支援能源績效改善",
                  "取代 ISO 50001 管理系統文件化要求",
                  "自動產生碳權供交易市場販售",
                  "完全取代現場運轉人員之操作判斷"
            ],
            "correct": 0,
            "explain": "EMS + AMI 提供即時監測與異常診斷，支援 PDCA 改善；不能取代管理系統文件或直接產生碳權。",
            "epRef": 11,
            "tag": [
                  "EMS",
                  "AMI"
            ]
      }
],
        "reinforcement": [
      {
            "id": "L212_R01",
            "type": "加強",
            "q": "關於 ESCO 三種合約模式的風險與資金歸屬，下列敘述何者正確？",
            "options": [
                  "Shared Saving 由業主出資，ESCO 保證節能量",
                  "Guaranteed Saving 由 ESCO 出資並承擔全部技術風險，業主僅保證按月付費",
                  "Chauffage 模式下 ESCO 負責提供能源服務並收取固定費用，承擔供能風險",
                  "三種模式下 ESCO 皆不承擔任何財務風險"
            ],
            "correct": 2,
            "explain": "Chauffage 為能源服務包乾制，ESCO 承擔供能效率風險；Shared 由 ESCO 出資，Guaranteed 由業主出資並由 ESCO 保證績效。",
            "epRef": 11,
            "tag": [
                  "ESCO"
            ]
      },
      {
            "id": "L212_R02",
            "type": "加強",
            "q": "某工廠去年導入節能措施後總用電下降 8%，但同期產量亦下降 12%。若僅以總用電變化評估節能績效，可能產生何種偏誤？",
            "options": [
                  "高估節能績效，因未將產量變化做標準化處理",
                  "低估節能績效，因產量下降會掩蓋實際改善成效",
                  "無偏誤，總用電下降即代表節能成功",
                  "僅須比較電費金額即可避免偏誤"
            ],
            "correct": 0,
            "explain": "產量下降本身會使總能耗降低，若未以單位產品耗能（EnPI）進行標準化，可能把營運萎縮誤認為節能改善。",
            "epRef": 11,
            "tag": [
                  "ISO50001",
                  "EnPI"
            ]
      },
      {
            "id": "L212_R03",
            "type": "加強",
            "q": "關於綠氫與藍氫的比較，下列敘述何者正確？",
            "options": [
                  "綠氫目前生產成本普遍低於藍氫，已全面取代化石來源氫能供應",
                  "藍氫依賴天然氣與 CCUS，綠氫則依賴再生能源電解設備",
                  "兩者碳排相同，差異僅在原料來源名稱",
                  "藍氫屬於零碳氫，綠氫因需用電反而碳排較高"
            ],
            "correct": 1,
            "explain": "藍氫為 SMR + CCUS，綠氫為再生能源電解。目前綠氫成本仍高於藍氫，藍氫非零碳（僅捕捉部分 CO2）。",
            "epRef": 11,
            "tag": [
                  "氫能"
            ]
      },
      {
            "id": "L212_R04",
            "type": "加強",
            "q": "比較鋰離子電池與液流電池的特性，下列敘述何者最正確？",
            "options": [
                  "鋰電池能量密度高、響應快，適合電動車與短中時應用",
                  "液流電池能量密度高於鋰電池，適合可攜式裝置與電動車應用",
                  "兩者循環壽命相當，應用場景可完全互換",
                  "鋰電池的功率與容量可獨立擴充，液流電池則否"
            ],
            "correct": 0,
            "explain": "鋰電池能量密度高但循環壽命較短；液流電池能量密度低但功率與容量可獨立設計、壽命長，適合固定式長時儲能。",
            "epRef": 11,
            "tag": [
                  "儲能"
            ]
      },
      {
            "id": "L212_R05",
            "type": "加強",
            "q": "關於 V2G（Vehicle-to-Grid）與傳統固定式儲能的差異，下列敘述何者正確？",
            "options": [
                  "V2G 利用電動車閒置時的電池雙向供電參與電網服務，具分散式特性",
                  "V2G 僅能單向由電網對車輛充電，無法回饋電力",
                  "V2G 與固定式儲能完全相同，只是名稱不同",
                  "V2G 僅限於再生能源場域使用，不得併入一般電網調度且無法參與輔助服務"
            ],
            "correct": 0,
            "explain": "V2G 的核心為雙向充放電，利用車輛閒置電池支援電網調頻與削峰，屬分散式資源；單向者稱 V1G。",
            "epRef": 11,
            "tag": [
                  "V2G",
                  "智慧能源"
            ]
      }
]
      },
      {
        "id": "L213",
        "name": "再生能源與綠電",
        "subject": "節能減碳技術實務",
        "scope": "再生能源種類比較（含海洋能）、T-REC、CPPA、自發自用",
        "examTips": "🔥 4/11 實考重點：再生能源定義與應用情境、綠電憑證 T-REC 轉移實務、PPA 購售電合約條款判讀",
        "concepts": [
          { "id": "re-types", "name": "再生能源種類與特性", "tags": ["風電","地熱","海洋能","生質能","容量因數"] },
          { "id": "green-cert", "name": "綠電憑證與交易", "tags": ["TREC","憑證","RE100","綠電主張","自發自用"] },
          { "id": "ppa", "name": "購售電合約實務", "tags": ["CPPA","Scope2"] },
          { "id": "re-finance", "name": "再生能源投資與儲能", "tags": ["NPV","儲能","SBTi","雙重計算"] }
        ],
        "frameworkQuestion": "潮汐、波浪、潮流、OTEC、鹽差五種海洋能的「可預測性」、「成熟度」、「裝置容量」如何排序？",
        "controversy": "T-REC 跟實體 PPA 都是綠電，為什麼大型企業寧可簽 CPPA 十年長約？",
        "frameworkQ": {
          "question": "潮汐、波浪、潮流、OTEC、鹽差五種海洋能的「可預測性」、「成熟度」、「裝置容量」如何排序？",
          "hint": "想想看：月球引力造成的潮汐 vs 風驅動的波浪，哪個更容易預測發電量？",
          "answer": "可預測性：潮汐 > 潮流 > OTEC > 波浪 > 鹽差。成熟度：潮汐壩 > 潮流渦輪 > 波浪 > OTEC > 鹽差。裝置容量：潮汐壩最大（法國 240MW），其餘多為示範級（<10MW）。iPAS 考試重點：潮汐可預測性高是最常考的比較項，海洋能整體商轉規模仍有限。"
        },
        "controversyQ": {
          "scenario": "T-REC 跟實體 PPA 都是綠電，為什麼大型企業寧可簽 CPPA 十年長約？",
          "sideA": { "label": "T-REC 就夠了", "argument": "T-REC 彈性高、門檻低，按年度採購即可。不需承擔長期合約風險，電價波動時可靈活調整。對 RE100 報告同樣有效，小企業首選。" },
          "sideB": { "label": "CPPA 才是正解", "argument": "CPPA 鎖定 10-20 年電價，對沖未來電價上漲風險。搭配 T-REC 同步取得，一站式解決合規。展現企業長期減碳承諾，ESG 評級加分。大型用電戶的經濟規模效益明顯。" },
          "insight": "核心差異是「風險偏好」和「用電規模」。年用電 >500 萬度的大戶簽 CPPA 鎖價划算；中小企業用 T-REC 保持彈性。兩者可組合使用：CPPA 覆蓋基載、T-REC 補尖峰。Scope 2 市場基礎法下兩者效果相同。"
        },
        "questions": [
          {
            "id": "L213_Q01",
            "type": "單選",
            "q": "關於海洋能與其他再生能源比較，何者正確？",
            "options": [
              "海洋能（潮汐、波浪）發電量大且完全不受地理限制，可在任何海岸部署",
              "海洋能可預測性高（潮汐），但受地理位置限制，商轉規模仍有限",
              "海洋能是目前台灣佔比最高的再生能源",
              "海洋能完全不需要電網併接考量"
            ],
            "correct": 1,
            "explain": "B 正確。A「完全不受地理限制」絕對語；C 事實錯（台灣主力為太陽能、風力）；D「完全不需」絕對語。",
            "epRef": 21,
            "tag": [
              "海洋能",
              "陷阱",
              "絕對語"
            ]
          },
          {
            "id": "L213_Q02",
            "type": "情境",
            "q": "某半導體廠 RE100 承諾 2030 年達標，年用電 50 億度，目前綠電採購 0%。最合適的綠電策略為？",
            "options": [
              "只買 T-REC 憑證即可滿足所有綠電需求，價格便宜且不受合約年限約束",
              "簽 CPPA 鎖定長期電量（10-20 年），輔以 T-REC 補短期缺口",
              "等 2029 年再一次買齊",
              "完全自建光電，不簽任何 PPA"
            ],
            "correct": 1,
            "explain": "大用電戶 RE100 需穩定長期供給，CPPA 為主、T-REC 為輔是實務常見配置。A 僅憑證不含實體電；C 延後風險；D 自建規模不足。",
            "epRef": 21,
            "tag": [
              "CPPA",
              "TREC",
              "情境"
            ]
          },
          {
            "id": "L213_Q03",
            "type": "單選",
            "q": "關於 T-REC（台灣再生能源憑證）移轉：",
            "options": [
              "T-REC 可無限次轉讓且抵用範圍不受時間限制，同一張憑證可供多家企業主張",
              "T-REC 具唯一序號、一次性使用（抵用後註銷），不得重複主張",
              "T-REC 只能由發電業使用，企業不得購買",
              "T-REC 與實體電力可分離，但抵用後仍可再轉售"
            ],
            "correct": 1,
            "explain": "B 正確，憑證抵用即註銷。A「無限次」錯；C 錯誤；D「抵用後仍可轉售」為雙重計算陷阱。",
            "epRef": 21,
            "tag": [
              "TREC",
              "陷阱"
            ]
          },
          {
            "id": "L213_Q04",
            "type": "情境",
            "q": "某企業用電 5,000 萬度/年，CPPA 綠電 4.5 元/度 vs 市電 3.5 元/度，綠電年溢價為？",
            "options": [
              "500 萬元",
              "5,000 萬元",
              "1 億元",
              "1.75 億元"
            ],
            "correct": 1,
            "explain": "溢價=5000 萬度×(4.5-3.5)=5,000 萬元/年。",
            "epRef": 21,
            "tag": [
              "CPPA",
              "計算"
            ]
          },
          {
            "id": "L213_Q05",
            "type": "情境",
            "q": "CPPA 與 T-REC 主要差異中，最關鍵的為？",
            "options": [
              "CPPA 僅涵蓋憑證移轉不含實體電力供應；T-REC 則同時涵蓋實體電力與憑證",
              "CPPA 含實體電力 + 憑證長期綁定；T-REC 為憑證移轉，可獨立於實體電",
              "兩者完全相同，僅名稱不同",
              "T-REC 一定比 CPPA 貴"
            ],
            "correct": 1,
            "explain": "B 為核心差異。A 定義顛倒；C/D 皆錯。",
            "epRef": 21,
            "tag": [
              "CPPA",
              "TREC",
              "陷阱"
            ]
          },
          {
            "id": "L213_Q06",
            "type": "單選",
            "q": "關於自發自用太陽光電：",
            "options": [
              "自發自用產生的綠電可同時申請 T-REC 憑證並售出，不影響自用抵減，因為自用與售憑是獨立系統",
              "自發自用如申請 T-REC 並售出，該部分電力不得再主張為自己的綠電使用（雙重計算）",
              "自發自用永遠比購買綠電便宜",
              "自發自用完全不需要電表驗證"
            ],
            "correct": 1,
            "explain": "B 正確，避免雙重計算。A 為典型陷阱；C「永遠」絕對語；D「完全不需要」絕對語。",
            "epRef": 21,
            "tag": [
              "自發自用",
              "雙重計算",
              "陷阱"
            ]
          },
          {
            "id": "L213_Q07",
            "type": "情境",
            "q": "儲能系統（ESS）搭配再生能源的主要效益為？",
            "options": [
              "取代再生能源發電",
              "平抑再生能源間歇性、削峰填谷、提供輔助服務",
              "降低 T-REC 價格並增加憑證供給量，使綠電主張門檻下降",
              "取代電網"
            ],
            "correct": 1,
            "explain": "儲能功能為平抑間歇性、削峰填谷、輔助服務，並非取代。",
            "epRef": 21,
            "tag": [
              "儲能",
              "情境"
            ]
          },
          {
            "id": "L213_Q08",
            "type": "單選",
            "q": "綠電採購對 Scope 2 排放計算（市場基礎法）的影響：",
            "options": [
              "只要實際使用綠電即可於 Scope 2 主張零排放，無須取得任何憑證或書面證明",
              "採購綠電並取得有效憑證（如 T-REC、I-REC）才能於市場基礎法主張為零排放",
              "綠電僅影響 Scope 1",
              "綠電對 Scope 2 完全沒影響"
            ],
            "correct": 1,
            "explain": "B 正確。A 缺憑證主張會被視為雙重計算；C/D 認知錯誤。",
            "epRef": 21,
            "tag": [
              "Scope2",
              "陷阱",
              "跨章"
            ]
          },
          {
            "id": "L213_Q09",
            "type": "情境",
            "q": "離岸風電 vs 陸域風電比較，何者正確？",
            "options": [
              "離岸風電容量因數較低且建置成本也低於陸域，適合小型試驗",
              "離岸容量因數較高但單位建置成本也較高，適合大型商轉",
              "陸域風電完全不受地理限制",
              "兩者對電網影響完全相同"
            ],
            "correct": 1,
            "explain": "B 正確。A 容量因數陳述錯；C/D 絕對語錯誤。",
            "epRef": 21,
            "tag": [
              "風電",
              "情境"
            ]
          },
          {
            "id": "L213_Q10",
            "type": "單選",
            "q": "關於地熱發電的特性，下列敘述何者正確？",
            "options": [
              "屬間歇性發電且受天候影響大，特性與太陽能類似",
              "為基載型再生能源，容量因數可達 70% 以上",
              "完全無環境影響",
              "只能用於發電，不能用於熱能"
            ],
            "correct": 1,
            "explain": "B 正確，地熱為基載型。A 錯誤（地熱非間歇）；C/D 絕對語誤。",
            "epRef": 21,
            "tag": [
              "地熱",
              "陷阱"
            ]
          },
          {
            "id": "L213_Q11",
            "type": "整合",
            "q": "某企業簽 20 年 CPPA 達 RE100，經測算年綠電溢價為 5,000 萬元。若以折現率 6% 評估此長期合約的成本現值，下列何者最接近正確答案？",
            "options": [
              "約 1 億元",
              "約 5.74 億元",
              "約 10 億元",
              "約 3 億元"
            ],
            "correct": 1,
            "explain": "5,000 萬×11.47≈5.735 億。C 為未折現直接 5000×20=10 億的陷阱。",
            "epRef": 21,
            "tag": [
              "跨章",
              "NPV",
              "CPPA"
            ]
          },
          {
            "id": "L213_Q12",
            "type": "整合",
            "q": "採購綠電於 SBTi 與 Scope 2 揭露時：",
            "options": [
              "綠電只能抵減 Scope 1",
              "綠電採購可於 Scope 2（市場基礎法）主張減量，且需揭露於 SBTi 進度報告",
              "綠電可同時用於抵減 Scope 1 直接排放與 Scope 3 供應鏈排放，不限於 Scope 2",
              "SBTi 不承認綠電採購"
            ],
            "correct": 1,
            "explain": "B 正確。綠電屬 Scope 2 市場基礎法範疇，SBTi 承認。",
            "epRef": 21,
            "tag": [
              "跨章",
              "SBTi",
              "Scope2"
            ]
          },
          {
            "id": "L213_Q13",
            "type": "情境",
            "q": "小型企業年用電 300 萬度，欲達 30% 綠電，預算有限，最務實的做法為？",
            "options": [
              "簽長期 CPPA",
              "購買 T-REC 憑證 90 萬度，彈性高、門檻低",
              "自建光電 90 萬度需大量土地面積，且建置期長達 3-5 年才能穩定供電",
              "等補助"
            ],
            "correct": 1,
            "explain": "小用量且預算有限，T-REC 為最務實門檻最低方案。",
            "epRef": 21,
            "tag": [
              "TREC",
              "情境"
            ]
          },
          {
            "id": "L213_Q14",
            "type": "單選",
            "q": "關於綠電類別（國際上）：",
            "options": [
              "I-REC 為國際通用憑證，T-REC 為台灣專用，兩者不可直接互換",
              "T-REC 可於全球任何國家主張綠電使用，不限台灣地區且與 I-REC 完全互通",
              "I-REC 僅限歐洲使用",
              "所有再生能源憑證通用"
            ],
            "correct": 0,
            "explain": "A 正確。B/C/D 皆為絕對語或事實錯誤。",
            "epRef": 21,
            "tag": [
              "憑證",
              "陷阱"
            ]
          },
          {
            "id": "L213_Q15",
            "type": "情境",
            "q": "太陽能電廠容量因數約為？（台灣平均）",
            "options": [
              "5-8%",
              "14-17%",
              "35-40%",
              "70% 以上"
            ],
            "correct": 1,
            "explain": "台灣太陽能容量因數約 14-17%；離岸風電約 40%+；地熱約 70%+。",
            "epRef": 21,
            "tag": [
              "容量因數",
              "情境"
            ]
          }
        ],
        "reinforcement": [
          {
            "id": "L213_R01",
            "type": "情境",
            "q": "RE100 與 SBTi 主要差異？",
            "options": [
              "RE100 聚焦 100% 再生電力；SBTi 聚焦科學化減碳路徑（含非電力排放）",
              "兩者完全相同",
              "SBTi 僅限電力業",
              "RE100 涵蓋 Scope 1 所有排放源的減量承諾，不僅限於電力使用端的綠化"
            ],
            "correct": 0,
            "explain": "A 正確，兩者互補但範疇不同。",
            "epRef": 21,
            "tag": [
              "RE100",
              "SBTi",
              "加強"
            ]
          },
          {
            "id": "L213_R02",
            "type": "單選",
            "q": "關於生質能利用，下列敘述何者正確？",
            "options": [
              "凡生質能一律視為碳中和，燃燒排放不需納入溫室氣體盤查清冊",
              "生質能是否碳中和取決於來源、生命週期排放，仍需 LCA 評估",
              "生質能完全不產生 PM2.5",
              "生質能只能發電不能產熱"
            ],
            "correct": 1,
            "explain": "B 正確。A/C 絕對語錯；D 事實錯。",
            "epRef": 21,
            "tag": [
              "生質能",
              "加強"
            ]
          },
          {
            "id": "L213_R03",
            "type": "情境",
            "q": "企業欲公開主張「使用 100% 綠電」，必要條件為？",
            "options": [
              "綠電量足額且憑證對應註銷，避免雙重計算",
              "只需內部宣稱",
              "完全不需憑證",
              "僅需安裝光電板即可主張綠電，無須憑證註銷程序"
            ],
            "correct": 0,
            "explain": "主張綠電需憑證註銷佐證，避免雙重計算。",
            "epRef": 21,
            "tag": [
              "綠電主張",
              "加強"
            ]
          },
          {
            "id": "L213_R04",
            "type": "整合",
            "q": "台灣綠電轉供與自發自用搭配儲能時：",
            "options": [
              "儲能可延長綠電時間彈性、提升自用率；轉供則須走電業制度",
              "儲能設施必然降低綠電憑證的市場價值，因為時移效果稀釋了尖峰綠電溢價",
              "自發自用無須任何法規遵循",
              "轉供完全不計費"
            ],
            "correct": 0,
            "explain": "A 正確。B/C/D 皆為絕對語或事實錯誤。",
            "epRef": 21,
            "tag": [
              "跨章",
              "儲能",
              "加強"
            ]
          },
          {
            "id": "L213_R05",
            "type": "計算",
            "q": "年用電 2,000 萬度，採購 500 萬度 T-REC（$2/度），年憑證支出為？",
            "options": [
              "500 萬元",
              "1,000 萬元",
              "4,000 萬元",
              "2,000 萬元"
            ],
            "correct": 1,
            "explain": "500 萬度×$2=1,000 萬元。",
            "epRef": 21,
            "tag": [
              "TREC",
              "計算"
            ]
          }
        ]
      },
      {
        "id": "L221",
        "name": "氣候法與國際揭露",
        "subject": "碳管理制度實務",
        "scope": "氣候變遷因應法三子法、國際揭露架構、倡議準則接軌",
        "examTips": "🔥 4/11 實考重點：氣候法 × IFRS/GRI/TCFD/SBTi 整合題、碳費申報責任與調整機制、產業費率差異判斷",
        "concepts": [
          { "id": "climate-law", "name": "氣候變遷因應法", "tags": ["氣候法","三子法","碳費","費率","門檻","台灣時程","適用範圍","過渡期","自主減量"] },
          { "id": "disclosure", "name": "國際揭露架構", "tags": ["IFRS","ISSB","TCFD","GRI","SASB","雙重重大性","情境分析"] },
          { "id": "sbti-cbam", "name": "國際倡議與碳邊境", "tags": ["SBTi","CBAM","額度交易","路徑","淨零","盤查"] }
        ],
        "frameworkQuestion": "氣候變遷因應法的三個子法（碳費、自主減量、溫室氣體減量方法）各自解決什麼問題？",
        "controversy": "台灣碳費 2026 開徵 $300/噸，是不是太低？跟 EU ETS $60-80 歐元比怎麼看？",
        "frameworkQ": {
          "question": "氣候變遷因應法的三個子法（碳費、自主減量、溫室氣體減量方法）各自解決什麼問題？",
          "hint": "想想看：碳費是「罰」、自主減量是「獎」、減量方法是「怎麼做」— 三者如何搭配？",
          "answer": "碳費子法：對排放大戶（年排 ≥2.5 萬噸）徵收碳費，「以價制量」驅動減碳。自主減量子法：提交減碳計畫並達標者可享碳費優惠費率，「以獎代罰」。減量方法子法：規範哪些技術、方法學可產生合格的減量額度，確保減碳品質。三者形成「收費 → 優惠誘因 → 方法規範」的完整制度閉環。"
        },
        "controversyQ": {
          "scenario": "台灣碳費 2026 開徵 $300/噸，是不是太低？跟 EU ETS $60-80 歐元比怎麼看？",
          "sideA": { "label": "太低了", "argument": "EU ETS 約 €60-80（TWD 2,000-2,700），台灣 $300 不到十分之一，完全沒有減碳驅動力。企業繳費了事比投資減碳便宜，形同「碳排贖罪券」。國際碳邊境（CBAM）以歐盟價格為基準，台灣低價碳費無法抵扣。" },
          "sideB": { "label": "循序漸進合理", "argument": "台灣製造業毛利低（5-15%），一步到位會造成產業出走。$300 是起步價，政策設計已包含逐年調升機制。搭配自主減量優惠費率（約 50-100/噸），先讓企業建立盤查能力再加碼。韓國 ETS 初期也從低價起步。" },
          "insight": "碳費高低不是唯一重點，「預期上漲路徑」才是。企業看的是 5-10 年後的碳成本，若確定每年調升 10-15%，現在投資減碳的 NPV 就會變正。關鍵是政府要給出可預期的碳費調升時程表，讓企業做長期投資決策。"
        },
        "questions": [
      {
            "id": "L221_Q01",
            "type": "情境",
            "q": "某鋼鐵廠年直接排放量達 4 萬噸 CO2e，欲申請較低費率並規劃 5 年減量路徑，同時評估將來自願減量專案抵減空間。此企業在送件時，最主要應依循哪一子法提出申請文件？",
            "options": [
                  "碳費收費辦法",
                  "自主減量計畫管理辦法",
                  "溫室氣體減量額度交易管理辦法",
                  "溫室氣體排放量盤查登錄管理辦法"
            ],
            "correct": 1,
            "explain": "要申請優惠費率的關鍵動作是『提出減量計畫』，屬於自主減量計畫管理辦法的核心。碳費收費辦法規範費率本身，額度交易辦法規範抵換機制，盤查辦法則是上游前置作業。陷阱在考生常以為『申請費率=碳費收費辦法』。",
            "epRef": 41,
            "tag": [
                  "氣候法",
                  "三子法"
            ]
      },
      {
            "id": "L221_Q02",
            "type": "情境",
            "q": "一家石化業者取得國內自願減量專案產出的額度，欲用於抵減其應繳碳費。關於此操作所依循的法規層級，下列何者最精確？",
            "options": [
                  "應依碳費收費辦法直接計算抵減額度",
                  "應依自主減量計畫管理辦法申請認可",
                  "應依溫室氣體減量額度交易管理辦法辦理額度移轉與使用",
                  "應依氣候變遷因應法母法直接執行即可，不需額外子法規範"
            ],
            "correct": 2,
            "explain": "自願減量額度的取得、移轉、註銷、抵換上限皆由額度交易管理辦法規範。碳費收費辦法只規定『可抵多少比例』的應用面，不規範額度本身。",
            "epRef": 41,
            "tag": [
                  "氣候法",
                  "額度交易"
            ]
      },
      {
            "id": "L221_Q03",
            "type": "情境",
            "q": "環境部針對某產業公告行業別減量基準，企業若欲主張符合此基準並適用較低費率，其主張應於哪一程序中被審查？",
            "options": [
                  "碳費收費辦法的年度申報程序",
                  "自主減量計畫管理辦法的計畫核定程序",
                  "額度交易管理辦法的額度認可程序，由碳交所審定",
                  "盤查登錄辦法的查驗程序"
            ],
            "correct": 1,
            "explain": "行業基準的達成與否屬於『減量計畫』的審查內容，須於計畫核定階段認定。收費辦法只在費率適用時引用此結果。",
            "epRef": 41,
            "tag": [
                  "氣候法",
                  "自主減量"
            ]
      },
      {
            "id": "L221_Q04",
            "type": "計算",
            "q": "某製造業年直接排放量為 6 萬噸 CO2e，已通過自主減量計畫並符合行業別基準，但未達 SBTi 1.5°C 路徑的更嚴格門檻。其碳費計算基礎（噸數與適用費率組合）最接近下列何者？",
            "options": [
                  "6 萬噸全額適用一般費率",
                  "扣除免費額度後餘額適用優惠費率 A，免費配額比照歐盟",
                  "全額 6 萬噸適用優惠費率 A，不扣除任何額度",
                  "全額 6 萬噸適用優惠費率 B"
            ],
            "correct": 2,
            "explain": "優惠費率適用者『不再扣除 2.5 萬噸免費額度』，免費扣除僅一般費率適用。此外未達 SBTi 較嚴格要求，故只能適用較寬鬆的優惠等級，非更低的優惠等級。陷阱在『優惠又扣除』的雙重好處誤解。",
            "epRef": 41,
            "tag": [
                  "碳費",
                  "費率"
            ]
      },
      {
            "id": "L221_Q05",
            "type": "情境",
            "q": "甲廠年排放 2.3 萬噸、乙廠年排放 2.8 萬噸，兩者皆為製造業直接排放源。關於 2026 年開徵後兩廠的碳費徵收對象認定，下列敘述何者正確？",
            "options": [
                  "兩廠皆須繳碳費，只是費率不同",
                  "甲廠不在徵收對象，乙廠須繳",
                  "兩廠均因未達門檻而免繳",
                  "徵收對象以間接排放計，兩廠皆須繳"
            ],
            "correct": 1,
            "explain": "現行收費對象鎖定年直接排放達門檻者，甲廠未達門檻不屬徵收對象（非『免費扣除』概念）。陷阱在混淆『徵收門檻』與『免費額度』：前者是『有沒有資格被徵』，後者是『被徵者可扣多少』。",
            "epRef": 41,
            "tag": [
                  "碳費",
                  "門檻"
            ]
      },
      {
            "id": "L221_Q06",
            "type": "觀念",
            "q": "關於國際永續揭露準則自 2024 年生效後，與 TCFD 建議之間的關係，下列敘述何者最準確？",
            "options": [
                  "該準則完全取代 TCFD 四大支柱，TCFD 即刻失效且企業不可再引用其框架進行揭露",
                  "該準則延續 TCFD 四大支柱架構，並由 ISSB 接手維護相關監督工作",
                  "該準則僅規範財務揭露，氣候議題仍由 TCFD 獨立維護",
                  "該準則與 TCFD 為平行體系，企業須同時符合兩者"
            ],
            "correct": 1,
            "explain": "ISSB 於準則生效後承接 TCFD 的監督角色，準則延用治理/策略/風險管理/指標目標四支柱。『即刻失效不可引用』與『平行體系同時符合』皆為陷阱說法。",
            "epRef": 41,
            "tag": [
                  "IFRS",
                  "TCFD"
            ]
      },
      {
            "id": "L221_Q07",
            "type": "情境",
            "q": "我國金管會規劃將國際永續揭露準則導入上市櫃公司。依目前分階段安排，大型企業與全體適用的兩個時點設計原則，下列何者最符合？",
            "options": [
                  "大型企業先行，之後全體適用，兩階段間隔數年",
                  "全體同步適用，不分企業規模大小，所有上市櫃公司同年度開始揭露",
                  "僅金融業適用，製造業不納入",
                  "採自願申請，無強制時程"
            ],
            "correct": 0,
            "explain": "金管會採『由大到小、分階段上路』。同步適用、僅金融業、純自願皆為誤解。考生需記住『先大後全』的設計哲學，具體年份可能調整。",
            "epRef": 41,
            "tag": [
                  "IFRS",
                  "台灣時程"
            ]
      },
      {
            "id": "L221_Q08",
            "type": "觀念",
            "q": "關於 GRI、SASB、ISSB 三套揭露體系的定位與雙重重大性概念，下列敘述何者正確？",
            "options": [
                  "GRI 強調企業對外部（環境社會）的影響，屬影響重大性取向",
                  "SASB 涵蓋所有產業通用指標，不具產業特定性，且不區分財務重大性",
                  "ISSB 要求同時呈現影響重大性與財務重大性",
                  "雙重重大性僅適用於上市公司，未上市公司無須考慮"
            ],
            "correct": 0,
            "explain": "GRI 取向為『Inside-out』影響重大性；SASB 為產業別特定指標（非通用）；ISSB 以財務重大性（Outside-in）為主，而非雙重；雙重重大性源自歐盟 CSRD，非僅限上市公司。",
            "epRef": 41,
            "tag": [
                  "GRI",
                  "SASB",
                  "ISSB"
            ]
      },
      {
            "id": "L221_Q09",
            "type": "情境",
            "q": "某電子業擬揭露其供應鏈範疇三排放並呼應產業別重點議題（如水資源、員工安全），同時需滿足國際投資人對財務面氣候資訊的要求。最合理的準則組合是？",
            "options": [
                  "僅採 GRI 單一準則即可涵蓋全部",
                  "僅採 ISSB 準則即可涵蓋全部",
                  "採 GRI 揭露影響面，搭配 ISSB（含延用 SASB 產業指標）揭露財務面",
                  "僅採 TCFD 四支柱即可取代全部揭露義務，不需額外適用 GRI 或 ISSB 準則"
            ],
            "correct": 2,
            "explain": "影響重大性面向由 GRI 覆蓋，財務重大性與產業指標由 ISSB（吸收 SASB）覆蓋。單一準則都不足以同時滿足雙面向的投資人與利害關係人需求。",
            "epRef": 41,
            "tag": [
                  "GRI",
                  "ISSB",
                  "雙重重大性"
            ]
      },
      {
            "id": "L221_Q10",
            "type": "觀念",
            "q": "關於 SBTi 近期（short-term）目標與長期（long-term）淨零目標的要求差異，下列敘述何者最準確？",
            "options": [
                  "兩者皆要求同樣的年減率，僅目標年度不同",
                  "近期目標涵蓋 5-10 年減量斜率，長期目標須對齊淨零並含中和剩餘排放",
                  "長期目標可不涵蓋範疇三，僅近期需納入",
                  "近期目標僅適用服務業的範疇一減量，長期目標僅適用製造業的全範疇淨零"
            ],
            "correct": 1,
            "explain": "SBTi Net-Zero Standard 要求近期目標（5-10 年內）定義減量斜率，長期目標須對齊 1.5°C 淨零，並處理剩餘排放的中和。範疇三納入與否依占比判定，非依期間區分。",
            "epRef": 41,
            "tag": [
                  "SBTi",
                  "淨零"
            ]
      },
      {
            "id": "L221_Q11",
            "type": "計算",
            "q": "某公司承諾 SBTi 1.5°C 路徑，以 2020 為基準年。若年減率依 1.5°C 情境套用線性減量，至 2030 年相對基準年的累積減量幅度最接近下列何者？",
            "options": [
                  "約 25%",
                  "約 42%",
                  "約 60%",
                  "約 75%"
            ],
            "correct": 1,
            "explain": "1.5°C 路徑年減 4.2%（線性），10 年累積約 42%。WB2°C 年減 2.5% 則約 25%。此為 SBTi 最核心的數字記憶，考試高頻。",
            "epRef": 41,
            "tag": [
                  "SBTi",
                  "路徑"
            ]
      },
      {
            "id": "L221_Q12",
            "type": "情境",
            "q": "某台灣螺絲製造商輸出產品至歐盟。關於 CBAM 的適用判斷，下列何者正確？",
            "options": [
                  "螺絲屬一般機械零件，不在 CBAM 涵蓋產品範圍內，僅原材料（鋼鐵）才需申報",
                  "鋼鐵下游產品可能納入 CBAM 範圍，須依歐盟公告的 CN Code 認定",
                  "CBAM 僅規範原料進口，成品加工件一律排除",
                  "只要產品不含水泥、鋁、肥料即不屬 CBAM"
            ],
            "correct": 1,
            "explain": "CBAM 以歐盟 CN Code 清單認定，鋼鐵及其部分下游產品（含螺絲、螺栓等）已納入範圍。以『不屬五大類原料』或『僅原料』判斷皆為陷阱。",
            "epRef": 41,
            "tag": [
                  "CBAM",
                  "適用範圍"
            ]
      },
      {
            "id": "L221_Q13",
            "type": "情境",
            "q": "關於 CBAM 過渡期與正式期的差異，下列敘述何者正確？",
            "options": [
                  "過渡期已需購買憑證，只是價格較低",
                  "過渡期僅需申報 Embedded Emissions，正式期起才須購買憑證並繳交",
                  "過渡期與正式期的碳排申報規定完全相同，差別僅在於企業熟悉度與查驗頻率",
                  "正式期可用任何國際碳權全額抵換憑證"
            ],
            "correct": 1,
            "explain": "過渡期為季報義務、無收費；正式期起須依 EE 購買 CBAM 憑證。過渡期付費、兩期相同、全額抵換皆為常見陷阱。",
            "epRef": 41,
            "tag": [
                  "CBAM",
                  "過渡期"
            ]
      },
      {
            "id": "L221_Q14",
            "type": "整合",
            "q": "某上市製造業（年排放 8 萬噸）規劃：對內申請國內碳費優惠費率、對外因應歐盟出口規範、同時向國際投資人揭露氣候資訊。下列串接邏輯何者最完整？",
            "options": [
                  "氣候法自主減量計畫對接 SBTi 路徑作為基準依據，揭露面以 ISSB（含 TCFD 四支柱）呈現策略與指標，CBAM 面則以產品 EE 申報",
                  "三項業務彼此獨立，無須互相引用",
                  "僅揭露 GRI 即可同時滿足三項需求",
                  "以 CDP 評分取代 ISSB 揭露義務，因 CDP 已涵蓋 TCFD 四支柱且評分機制較國際化，同時可免除 CBAM 碳排申報要求"
            ],
            "correct": 0,
            "explain": "整合邏輯：減量計畫（氣候法）→ 以 SBTi 為路徑依據 → ISSB/TCFD 揭露策略指標 → CBAM 申報產品 EE。三者共用盤查與減量基礎資料。CDP 是自願揭露工具，不取代法定義務。",
            "epRef": 41,
            "tag": [
                  "整合",
                  "氣候法",
                  "IFRS",
                  "CBAM"
            ]
      },
      {
            "id": "L221_Q15",
            "type": "整合",
            "q": "某企業於年度永續報告書中揭露：已完成範疇一、二盤查並經第三方查驗、承諾 SBTi 近期目標、取得自願減量額度欲抵減碳費。關於揭露內容與法遵對接的敘述，何者最精確？",
            "options": [
                  "自願減量額度可全額抵減應繳碳費",
                  "自願減量額度抵減有法定上限，國內與國外來源各有不同比例限制",
                  "SBTi 承諾即等同通過核准審查，企業可直接據此主張優惠費率而無需額外查驗",
                  "範疇一、二盤查不須第三方查驗即可申報碳費"
            ],
            "correct": 1,
            "explain": "額度抵減有法定上限（國內、國外比例不同），非全額；SBTi『承諾』與『核准』是兩階段，僅承諾不等於核准；碳費申報需以查驗後排放量為準。此題整合 L221（法規）× L222（碳費操作）× L211（盤查）。",
            "epRef": 41,
            "tag": [
                  "整合",
                  "碳費",
                  "SBTi",
                  "盤查"
            ]
      }
],
        "reinforcement": [
      {
            "id": "L221_R01",
            "type": "加強",
            "q": "關於自主減量計畫的申請條件與計畫期效，下列敘述何者正確？",
            "options": [
                  "任何排放源皆可申請自主減量計畫，無排放量門檻限制且計畫期間無年限上限",
                  "申請者須達行業別基準或對應國際科學基礎路徑，計畫期有年限上限並屆期須檢討",
                  "計畫一經核定永久有效，不需再檢討",
                  "計畫僅適用服務業，製造業另有專屬管道"
            ],
            "correct": 1,
            "explain": "自主減量計畫以『達行業基準或 SBTi 等科學路徑』為核心門檻，期滿須檢討更新。永久有效、行業限制皆為陷阱。",
            "epRef": 41,
            "tag": [
                  "自主減量"
            ]
      },
      {
            "id": "L221_R02",
            "type": "加強",
            "q": "關於 CBAM 過渡期與正式期之關鍵差異，下列何者為非？",
            "options": [
                  "過渡期採季度申報，無憑證購買義務",
                  "正式期須依 EE 購買並繳交憑證",
                  "過渡期與正式期對 EE 計算方法要求完全一致，不因階段調整",
                  "正式期對預設值的使用限制將比過渡期嚴格，且不允許使用來源國的排放係數"
            ],
            "correct": 2,
            "explain": "過渡期允許較寬鬆的預設值使用，正式期逐步限縮並要求實測數據。EE 方法在兩期有差異，並非完全一致。",
            "epRef": 41,
            "tag": [
                  "CBAM"
            ]
      },
      {
            "id": "L221_R03",
            "type": "加強",
            "q": "企業進行 TCFD 情境分析時，常採 1.5°C 與 3°C 兩情境並陳。此設計的核心目的最接近下列何者？",
            "options": [
                  "同時檢視轉型風險（嚴格政策）與實體風險（氣候失控）的雙向衝擊",
                  "僅為符合揭露字數要求",
                  "1.5°C 與 3°C 情境分析結果相同，差別只在呈現格式與報告模板不同",
                  "兩情境僅適用能源業"
            ],
            "correct": 0,
            "explain": "低溫情境放大轉型風險（碳定價、法規），高溫情境放大實體風險（極端天氣、資產損失）。雙情境可避免企業僅看單一面向而低估風險。",
            "epRef": 41,
            "tag": [
                  "TCFD",
                  "情境分析"
            ]
      },
      {
            "id": "L221_R04",
            "type": "加強",
            "q": "關於雙重重大性（Double Materiality）的兩個維度，下列敘述何者正確？",
            "options": [
                  "指財務年度與永續年度兩個時間維度的報告週期差異，非影響面與財務面的區分",
                  "指影響重大性（企業對環境社會影響）與財務重大性（環境社會對企業財務影響）",
                  "指母公司與子公司兩層組織維度",
                  "指本國與跨國兩個地理維度"
            ],
            "correct": 1,
            "explain": "雙重重大性源自歐盟 CSRD，強調 Inside-out（企業對外影響）與 Outside-in（外部對企業財務影響）兩面向。GRI 偏前者，ISSB 偏後者。",
            "epRef": 41,
            "tag": [
                  "雙重重大性"
            ]
      },
      {
            "id": "L221_R05",
            "type": "加強",
            "q": "關於 SBTi『承諾（Commit）』與『核准（Validated/Approved）』的差異，下列敘述何者正確？",
            "options": [
                  "兩者意義相同，承諾與核准皆代表企業減碳目標已通過 SBTi 正式審查並公開登錄",
                  "承諾僅表達意向並登錄，核准則是目標經 SBTi 審查通過；企業需在時限內提交目標以免承諾失效",
                  "承諾已具法律效力，核准僅為行銷用途",
                  "核准後目標不再需要更新"
            ],
            "correct": 1,
            "explain": "承諾是第一步意向登錄，需於期限（通常 24 個月）內提交具體目標受審並取得核准，否則地位撤銷。核准後若情境重大變動仍須更新。",
            "epRef": 41,
            "tag": [
                  "SBTi"
            ]
      }
]
      },
      {
        "id": "L222",
        "name": "自願減量實務",
        "subject": "碳管理制度實務",
        "scope": "國內外自願減量、CCUS、碳匯、碳抵換、ICP、碳資產管理",
        "examTips": "🔥 4/11 實考重點：SBTi 鋼鐵/水泥年減率、碳中和指引抵換時間限制、內部碳定價實務、碳費扣減機制陷阱",
        "concepts": [
          { "id": "carbon-pricing", "name": "碳費與減量機制", "tags": ["碳費","抵減上限","法規門檻","法規","自主減量"] },
          { "id": "sbti-path", "name": "SBTi 路徑與目標", "tags": ["SBTi","SDA","年減率","Scope3","Scope歸屬","歸屬判斷"] },
          { "id": "offset", "name": "碳權與抵換", "tags": ["碳中和","碳權分類","碳權類型","抵換","額外性","時效"] },
          { "id": "integration", "name": "碳資產整合管理", "tags": ["CBAM","揭露","整合","碳資產","碳中和"] }
        ],
        "frameworkQuestion": "17 萬噸排放的企業，用國內碳權抵 10%、國外碳權抵 5% 上限，實際碳費怎麼算？",
        "controversy": "自願碳市場 Verra VCS 爭議頻發，企業還該不該買？IFRS S2 要求揭露到什麼程度？",
        "frameworkQ": {
          "question": "17 萬噸排放的企業，用國內碳權抵 10%、國外碳權抵 5% 上限，實際碳費怎麼算？",
          "hint": "想想看：先扣 2.5 萬噸免徵額，再算抵換上限，最後乘費率 — 順序對嗎？",
          "answer": "不扣免徵額（台灣目前無免費配額制度）。17 萬噸全額計費。國內碳權抵 10% = 1.7 萬噸，國外碳權抵 5% = 0.85 萬噸，合計最多抵 2.55 萬噸。剩餘 14.45 萬噸 × $300 = 4,335 萬元。若通過自主減量計畫，可適用優惠費率（約 $50-100/噸），碳費大幅下降至 722-1,445 萬元。"
        },
        "controversyQ": {
          "scenario": "自願碳市場 Verra VCS 爭議頻發，企業還該不該買？IFRS S2 要求揭露到什麼程度？",
          "sideA": { "label": "謹慎購買", "argument": "Verra 的 REDD+ 林業碳權被揭露高估減碳量 90%+。SBTi 2024 起禁止用碳權抵 Scope 1/2 目標。買了有聲譽風險，被 NGO 質疑漂綠。IFRS S2 要求揭露碳權品質和方法學，透明度壓力大。" },
          "sideB": { "label": "策略性使用", "argument": "碳權用於「殘餘排放」中和是 SBTi 允許的。高品質碳權（如直接碳捕獲 DAC）仍被認可。IFRS S2 揭露不是禁止，而是要求說清楚。對難減碳行業（航空、水泥），碳權是唯一短期選項。" },
          "insight": "關鍵區分：碳權「抵減」（offset，用於合規或 SBTi）vs「中和」（neutralization，用於殘餘排放碳中和）。前者受限越來越嚴，後者仍被接受但要求品質。選購時看：額外性證明、永久性風險、第三方查驗、方法學是否被 ICVCM 認證。"
        },
        "questions": [
          {
            "id": "L222_Q01",
            "type": "計算",
            "q": "A 廠年排 17 萬噸 CO2e，購入國內碳權 15,000 噸、國外碳權 10,000 噸抵減，費率 300 元/噸。應繳碳費約為？",
            "options": [
              "4,500 萬元（未套抵減上限）",
              "4,395 萬元",
              "5,100 萬元（完全不抵）",
              "3,900 萬元（兩者皆未超上限）"
            ],
            "correct": 1,
            "explain": "國內上限 17,000 噸（未超）、國外上限 8,500 噸（實購 10,000 超上限，只能抵 8,500）。(170000-15000-8500)×300=146,500×300=43,950,000。",
            "epRef": 31,
            "tag": [
              "碳費",
              "計算",
              "抵減上限"
            ]
          },
          {
            "id": "L222_Q02",
            "type": "單選",
            "q": "台灣碳費制度的抵減上限：",
            "options": [
              "國內碳權抵 5%、國外 10%（順序顛倒）",
              "國內碳權抵 10%、國外抵 5%",
              "國內外皆可抵 20%",
              "完全不設上限"
            ],
            "correct": 1,
            "explain": "正確為國內 10%、國外 5%。A 順序顛倒是常見陷阱；C/D 錯。",
            "epRef": 31,
            "tag": [
              "碳費",
              "陷阱",
              "法規"
            ]
          },
          {
            "id": "L222_Q03",
            "type": "計算",
            "q": "B 廠年排 3 萬噸，若適用自主減量計畫優惠費率 50 元/噸（vs 一般 300 元/噸），年省碳費為？",
            "options": [
              "150 萬元",
              "750 萬元",
              "900 萬元（整體一般費率成本）",
              "1,500 萬元"
            ],
            "correct": 1,
            "explain": "年省=30,000×(300-50)=30,000×250=7,500,000。C 為總成本非省下金額。",
            "epRef": 31,
            "tag": [
              "碳費",
              "自主減量",
              "計算"
            ]
          },
          {
            "id": "L222_Q04",
            "type": "單選",
            "q": "關於台灣碳費徵收門檻，下列敘述何者正確？",
            "options": [
              "10,000 tCO2e/年",
              "25,000 tCO2e/年",
              "100,000 tCO2e/年",
              "無門檻，全面徵收"
            ],
            "correct": 1,
            "explain": "首波徵收對象為年排 25,000 tCO2e 以上的電力、製造業。",
            "epRef": 31,
            "tag": [
              "碳費",
              "法規門檻"
            ]
          },
          {
            "id": "L222_Q05",
            "type": "情境",
            "q": "某公司取得 2018 年簽發的碳權，欲用於 2026 年宣告碳中和。下列何者正確？",
            "options": [
              "任何年份簽發的碳權皆可用於碳中和主張，不受簽發日期或有效期限制",
              "依碳中和指引，碳權簽發年期有時效限制（通常 5 年內），過舊碳權不被接受",
              "只要是國際認證的碳權永遠有效",
              "碳權無時效概念"
            ],
            "correct": 1,
            "explain": "碳中和主張要求碳權近期簽發（多為 5 年內），避免陳舊額度問題。A/C/D 為絕對語陷阱。",
            "epRef": 31,
            "tag": [
              "碳中和",
              "時效",
              "陷阱"
            ]
          },
          {
            "id": "L222_Q06",
            "type": "情境",
            "q": "SBTi 1.5°C 路徑下，企業設定絕對減量目標的建議年減率為？",
            "options": [
              "2.5%",
              "4.2%",
              "6%",
              "10%"
            ],
            "correct": 1,
            "explain": "SBTi 1.5°C 絕對收縮法要求年減約 4.2%（至 2030 年前）。",
            "epRef": 31,
            "tag": [
              "SBTi",
              "年減率"
            ]
          },
          {
            "id": "L222_Q07",
            "type": "計算",
            "q": "鋼鐵廠基準年排 100 萬噸，依 SBTi 1.5°C 年減 4.2%（線性絕對收縮），10 年後目標排放為？",
            "options": [
              "42 萬噸",
              "58 萬噸",
              "66 萬噸（幾何減量誤算）",
              "90 萬噸"
            ],
            "correct": 1,
            "explain": "線性=100×(1-0.042×10)=58 萬噸。",
            "epRef": 31,
            "tag": [
              "SBTi",
              "計算"
            ]
          },
          {
            "id": "L222_Q08",
            "type": "單選",
            "q": "自願性碳權（VCS、GS 等）與合規性碳權差異：",
            "options": [
              "自願性碳權與合規性碳權可完全互換使用，且在任何法域下均具同等法律效力",
              "自願性碳權用於自願主張（如碳中和），合規性碳權用於法規義務抵減，抵減規則不同",
              "合規性碳權永遠比自願碳權便宜",
              "自願碳權不需第三方查驗"
            ],
            "correct": 1,
            "explain": "B 正確。A「完全互換」錯；C「永遠便宜」絕對語；D 錯（需查驗）。",
            "epRef": 31,
            "tag": [
              "碳權分類",
              "陷阱"
            ]
          },
          {
            "id": "L222_Q09",
            "type": "情境",
            "q": "下列何種情境最適合使用「移除類（Removal）」而非「減量類（Avoidance）」碳權？",
            "options": [
              "企業短期補足少量剩餘排放且高度成本敏感，優先選擇市場上最便宜的抵換額度即可",
              "企業追求科學化長期淨零、欲抵「殘餘排放」（residual emissions）",
              "取代電費支出",
              "取代 SBTi 減量路徑"
            ],
            "correct": 1,
            "explain": "移除類（如造林、DAC）適於抵殘餘排放，符合淨零目標；減量類則多用於過渡期。",
            "epRef": 31,
            "tag": [
              "碳權類型",
              "情境"
            ]
          },
          {
            "id": "L222_Q10",
            "type": "單選",
            "q": "關於「碳抵換（offset）」主張：",
            "options": [
              "只要買碳權即可主張碳中和，無須優先減量",
              "應優先減量（Avoid-Reduce），殘餘才用抵換，且須符合額外性、永久性原則",
              "抵換可同時用於 SBTi 路徑承諾與法規碳費抵減，不計上限且不要求額外性證明",
              "抵換後碳權可重複使用"
            ],
            "correct": 1,
            "explain": "B 符合減量層級原則與碳權四大要件（額外性、永久性、可量化、不雙重計算）。",
            "epRef": 31,
            "tag": [
              "抵換",
              "陷阱"
            ]
          },
          {
            "id": "L222_Q11",
            "type": "情境",
            "q": "水泥業依 SBTi 產業別路徑（SDA）設定強度目標，主要驅動力為？",
            "options": [
              "企業規模",
              "產業整體減碳斜率與產品基準（如 tCO2/t 熟料）",
              "碳權市場價格走勢，因為減碳斜率應參考碳交易市場供需決定",
              "員工人數"
            ],
            "correct": 1,
            "explain": "SDA（產業去碳化法）以產業脫碳斜率與產品強度基準制定目標。",
            "epRef": 31,
            "tag": [
              "SBTi",
              "SDA",
              "情境"
            ]
          },
          {
            "id": "L222_Q12",
            "type": "整合",
            "q": "A 廠年排 2 萬噸（未達 25,000 門檻），但客戶要求 SBTi 揭露。該廠應：",
            "options": [
              "因未達碳費門檻故無須進行任何減碳規劃，也不需要揭露排放數據或回應供應鏈要求",
              "雖未達碳費門檻，仍可自願提 SBTi、揭露 Scope 1/2/3，回應供應鏈要求",
              "等法規強制再做",
              "只揭露 Scope 1 即可"
            ],
            "correct": 1,
            "explain": "法規門檻與供應鏈揭露要求分屬不同層次，SBTi 為自願性但供應鏈壓力驅動。",
            "epRef": 31,
            "tag": [
              "跨章",
              "SBTi",
              "法規"
            ]
          },
          {
            "id": "L222_Q13",
            "type": "單選",
            "q": "範疇三（Scope 3）排放包含下列何項？",
            "options": [
              "自有車隊燃油屬 Scope 1 直接排放，非 Scope 3 間接排放",
              "員工商務差旅、上下游運輸、採購商品的隱含排放",
              "自用電力（Scope 2）",
              "完全不含下游使用階段"
            ],
            "correct": 1,
            "explain": "B 為 Scope 3 典型 15 類別範圍。A 屬 Scope 1；C 屬 Scope 2；D「完全不含」絕對語錯。",
            "epRef": 31,
            "tag": [
              "Scope3",
              "陷阱",
              "跨章"
            ]
          },
          {
            "id": "L222_Q14",
            "type": "情境",
            "q": "企業租賃辦公室（僅租）用電排放，歸屬於租戶的？",
            "options": [
              "Scope 1",
              "Scope 2（若租戶承擔電費及控制權）或 Scope 3 類別 8（若房東控制）",
              "一律歸入 Scope 3 類別 8 上游租賃資產，不論租戶或房東誰承擔電費控制權",
              "完全不歸租戶"
            ],
            "correct": 1,
            "explain": "依財務/營運控制權判斷。實務上需視合約與電費歸屬，此為 4/11 實考點。",
            "epRef": 31,
            "tag": [
              "Scope歸屬",
              "陷阱",
              "情境"
            ]
          },
          {
            "id": "L222_Q15",
            "type": "情境",
            "q": "合資企業 50% 持股且無營運控制權，依股權比例法，該合資公司排放應如何納入母公司？",
            "options": [
              "全部 100% 納入",
              "依持股 50% 比例納入",
              "完全不納入",
              "僅納入 Scope 3 類別 15 投資，不計入 Scope 1 或 2"
            ],
            "correct": 1,
            "explain": "股權比例法依持股比例納入；若採營運控制法且無控制則不納。題目明確用股權比例法。",
            "epRef": 31,
            "tag": [
              "歸屬判斷",
              "情境"
            ]
          },
          {
            "id": "L222_Q16",
            "type": "單選",
            "q": "關於碳權「額外性（Additionality）」：",
            "options": [
              "只要專案實際產生可量測的減碳量即自動具備額外性，無需證明 BAU 基線外之增量",
              "須證明若無碳權收入，該減碳行動不會發生（Business-as-usual 情境外）",
              "額外性與基準線無關",
              "政府法規要求的減碳也具額外性"
            ],
            "correct": 1,
            "explain": "B 為標準定義。A/C/D 皆為常見陷阱（法規要求者不具額外性）。",
            "epRef": 31,
            "tag": [
              "額外性",
              "陷阱"
            ]
          },
          {
            "id": "L222_Q17",
            "type": "整合",
            "q": "C 廠年排 5 萬噸，自主減量計畫承諾年減 4.2%（SBTi 1.5°C），並購 3,000 噸國內碳權抵減。法規抵減上限及適用費率為？",
            "options": [
              "國內抵上限 10%=5,000 噸（3,000 未超），若計畫達標可適用優惠費率",
              "國內上限 5%、適用一般費率",
              "完全不可抵",
              "國內碳權抵減上限為 20%（即 10,000 噸），且自動適用優惠費率 A 而無需提交計畫"
            ],
            "correct": 0,
            "explain": "國內抵上限 10% 即 5,000 噸，3,000 未超。自主減量計畫達標適用優惠費率（如 50 元/噸）。",
            "epRef": 31,
            "tag": [
              "跨章",
              "法規",
              "碳費"
            ]
          }
        ],
        "reinforcement": [
          {
            "id": "L222_R01",
            "type": "計算",
            "q": "D 廠年排 30 萬噸，買國內 30,000 噸、國外 20,000 噸，費率 300 元/噸。應繳？",
            "options": [
              "6,750 萬元",
              "7,650 萬元",
              "9,000 萬元",
              "5,100 萬元"
            ],
            "correct": 1,
            "explain": "國內上限 10%=30,000（等於實購，全抵）、國外上限 5%=15,000（實購 20,000 超，只抵 15,000）。(300000-30000-15000)×300=255,000×300=76,500,000=7,650 萬。",
            "epRef": 31,
            "tag": [
              "碳費",
              "計算",
              "加強"
            ]
          },
          {
            "id": "L222_R02",
            "type": "單選",
            "q": "SBTi 揭露要求：",
            "options": [
              "僅需揭露 Scope 1",
              "需揭露 Scope 1+2，Scope 3 達門檻（佔總排 40%）亦須設目標",
              "完全不必揭露 Scope 3 排放數據，因 SBTi 近期目標僅涵蓋 Scope 1 與 Scope 2",
              "僅需每 10 年揭露一次"
            ],
            "correct": 1,
            "explain": "B 正確。SBTi 對 Scope 3 佔比 40% 以上者要求設目標。",
            "epRef": 31,
            "tag": [
              "SBTi",
              "揭露",
              "加強"
            ]
          },
          {
            "id": "L222_R03",
            "type": "情境",
            "q": "宣告「碳中和產品」需遵守？",
            "options": [
              "僅由公司內部宣稱即可達成碳中和，不需第三方查驗或依循國際標準流程",
              "依 PAS 2060 或相關指引，需量化、減量、抵換、查驗四步驟",
              "只要買碳權即可",
              "不需第三方確證"
            ],
            "correct": 1,
            "explain": "PAS 2060 或 ISO 14068 要求四步驟並第三方確證。",
            "epRef": 31,
            "tag": [
              "碳中和",
              "加強"
            ]
          },
          {
            "id": "L222_R04",
            "type": "整合",
            "q": "企業面對歐盟 CBAM（碳邊境調整機制）時：",
            "options": [
              "只繳台灣碳費即可滿足所有碳定價義務，歐盟 CBAM 不影響出口產品的碳成本",
              "出口至歐盟產品需申報隱含排放；台灣已付碳費部分可於 CBAM 扣減",
              "CBAM 僅針對歐盟企業",
              "完全與 Scope 3 無關"
            ],
            "correct": 1,
            "explain": "B 正確。CBAM 避免雙重課稅，已付碳費可扣抵，且涉及供應鏈 Scope 3 揭露。",
            "epRef": 31,
            "tag": [
              "CBAM",
              "跨章",
              "加強"
            ]
          },
          {
            "id": "L222_R05",
            "type": "單選",
            "q": "關於自主減量計畫（台灣）：",
            "options": [
              "不需實質達成減量目標，只要提交自主減量計畫文件即可持續享有優惠費率",
              "須承諾達指定減量目標（對應 SBTi 或國家路徑），達標才維持優惠費率",
              "一次申請永久適用",
              "無須查驗"
            ],
            "correct": 1,
            "explain": "B 正確，未達標將追補差額。A/C/D 為常見誤解。",
            "epRef": 31,
            "tag": [
              "自主減量",
              "加強"
            ]
          }
        ]
      },
      {
        "id": "L223",
        "name": "永續供應鏈管理",
        "subject": "碳管理制度實務",
        "scope": "範疇三 15 類、供應鏈減碳風險評估",
        "examTips": "🔥 4/11 實考重點：Scope 3 排放歸屬判斷（高頻陷阱）、GHG Protocol 分類、永續供應鏈評估實務",
        "concepts": [
          { "id": "scope3-class", "name": "Scope 3 分類與歸屬", "tags": ["Scope3","控制權","租賃","差旅","通勤","合資","加盟","組織邊界","運輸","類11","類15"] },
          { "id": "data-method", "name": "數據品質與估算方法", "tags": ["估算方法","混合法","熱點分析","數據品質","market-based","Scope2","雙重計算"] },
          { "id": "supplier-mgmt", "name": "供應商分級管理", "tags": ["供應商管理","分級","CDP","產業"] },
          { "id": "asset-link", "name": "碳資產與投資整合", "tags": ["整合","L222","碳資產","投資","金融"] }
        ],
        "frameworkQuestion": "範疇三 15 類哪些必盤、哪些可豁免？估算法 vs 實際數據如何取捨？",
        "controversy": "範疇三佔企業碳排 80%+，但供應商給不出數據時，估算值能不能當減量證明？",
        "frameworkQ": {
          "question": "範疇三 15 類哪些必盤、哪些可豁免？估算法 vs 實際數據如何取捨？",
          "hint": "想想看：如果你是製造業，類 1（採購）和類 11（產品使用）哪個排放量更大？",
          "answer": "SBTi 要求：Scope 3 佔總排放 40%+ 的類別必須設目標。製造業通常熱點在類 1（採購）、類 4/9（運輸）、類 11（產品使用）。金融業在類 15（投資）。數據取捨：核心供應商（排放前 20 大）用供應商特定法，其餘用支出法或產業平均。GHG Protocol 建議：重大排放源用主要數據、次要項用資料庫、低重要性用預設值。"
        },
        "controversyQ": {
          "scenario": "範疇三佔企業碳排 80%+，但供應商給不出數據時，估算值能不能當減量證明？",
          "sideA": { "label": "估算值不可靠", "argument": "用支出法估算的 Scope 3 誤差可達 ±50%，無法作為減量基準。供應商換了但支出金額不變，估算值不會反映真實減碳。查驗機構不認可高度估算值作為目標追蹤依據。" },
          "sideB": { "label": "先估算再迭代", "argument": "等所有供應商提供一級數據不切實際（中小供應商佔 70%+）。GHG Protocol 允許混合法：熱點用一級數據、其餘用估算。SBTi 也接受「提升數據品質」作為階段性目標。完美數據不如行動。" },
          "insight": "正確做法是「分級管理」：Top 20 排放供應商要求一級數據（佔 Scope 3 約 70-80%），其餘用估算。每年提升一級數據覆蓋率作為 KPI。估算值可用於識別熱點和排序，但不能單獨用於主張精確的減量百分比。"
        },
        "questions": [
      {
            "id": "L223_Q01",
            "type": "情境",
            "q": "某物流公司擁有 50 輛貨車，其中 30 輛由公司駕駛自行營運配送自家貨物,另 20 輛長期出租給外部客戶使用,租約中公司不介入車輛調度。這 20 輛出租車輛產生的燃料排放,在該公司的溫室氣體盤查中應如何歸屬?",
            "options": [
                  "納入 Scope 1,因車輛所有權屬於公司",
                  "納入 Scope 3 類別 13 下游租賃資產",
                  "納入 Scope 3 類別 8 上游租賃資產，因使用權歸屬承租方",
                  "納入 Scope 3 類別 4 上游運輸配送"
            ],
            "correct": 1,
            "explain": "車輛由公司持有並出租給外部使用,公司無營運控制權,依 GHG Protocol 應歸屬下游租賃資產(類 13),而非自身 Scope 1。",
            "epRef": 61,
            "tag": [
                  "Scope3",
                  "租賃"
            ]
      },
      {
            "id": "L223_Q02",
            "type": "情境",
            "q": "某科技公司長期承租一棟辦公大樓,租約載明公司對該建築之能源使用具完全營運控制權,且自行繳納電費與簽訂電力合約。該辦公大樓的用電所產生的間接排放,應如何分類?",
            "options": [
                  "Scope 3 類別 8 上游租賃資產",
                  "Scope 2 外購電力",
                  "Scope 3 類別 13 下游租賃資產",
                  "Scope 1 直接排放"
            ],
            "correct": 1,
            "explain": "承租人具營運控制權且自行採購電力時,應納入自身 Scope 2,而非以上游租賃形式歸入 Scope 3 類 8。",
            "epRef": 61,
            "tag": [
                  "Scope2",
                  "租賃",
                  "控制權"
            ]
      },
      {
            "id": "L223_Q03",
            "type": "情境",
            "q": "某顧問公司員工一年內有 200 次高鐵出差、50 次搭乘飛機赴海外會議、另有 30 次使用公司配發之公務車往返客戶端。以下何者對這些出行活動的 Scope 歸屬描述最正確?",
            "options": [
                  "全部納入 Scope 3 類別 6 商務差旅",
                  "高鐵與飛機屬 Scope 3 類 6,公務車屬 Scope 1",
                  "全部納入 Scope 1,因屬員工執行公務",
                  "高鐵飛機屬 Scope 3 類別 7 員工通勤,公務車屬 Scope 1 直接排放"
            ],
            "correct": 1,
            "explain": "公司車燃料直接受公司控制屬 Scope 1;搭乘外部運輸工具(高鐵、飛機)屬差旅類別 6;類 7 為員工通勤,與差旅不同。",
            "epRef": 61,
            "tag": [
                  "Scope3",
                  "差旅",
                  "通勤"
            ]
      },
      {
            "id": "L223_Q04",
            "type": "情境",
            "q": "某公司對員工提供通勤補助,員工每日自駕或搭乘大眾運輸往返公司與住家。這部分因員工通勤所衍生的溫室氣體排放,應歸入何者?",
            "options": [
                  "Scope 3 類別 6 商務差旅",
                  "Scope 3 類別 4 上游運輸配送",
                  "Scope 3 類別 7 員工通勤",
                  "Scope 1,因公司已提供通勤補助"
            ],
            "correct": 2,
            "explain": "員工往返住家與辦公場所的日常通勤屬於獨立類別,與商務差旅(類 6)及運輸配送(類 4)需加以區分。",
            "epRef": 61,
            "tag": [
                  "Scope3",
                  "通勤"
            ]
      },
      {
            "id": "L223_Q05",
            "type": "情境",
            "q": "甲公司持有乙合資企業 40% 股權,依合資協議,甲公司對乙之財務與營運政策均無主導權。甲公司採用股權比例法進行盤查時,對乙合資企業之排放應如何處理?",
            "options": [
                  "不納入甲公司盤查,因無控制權",
                  "全額納入甲公司 Scope 1 與 Scope 2，因為合資企業應以經營控制權全額認列",
                  "依 40% 比例納入甲公司相對應的 Scope 1 與 Scope 2",
                  "全額納入甲公司 Scope 3 類別 15"
            ],
            "correct": 2,
            "explain": "採股權比例法時,按股權比例認列合資方之直接與能源間接排放;若改採營運控制權法則視控制情況處理。",
            "epRef": 61,
            "tag": [
                  "合資",
                  "組織邊界"
            ]
      },
      {
            "id": "L223_Q06",
            "type": "情境",
            "q": "某製造商將原料從供應商工廠運至自家廠區之運輸作業,委外給第三方物流業者執行,運費由製造商支付。此段運輸排放對製造商而言應歸屬於?",
            "options": [
                  "Scope 3 類別 4 上游運輸與配送",
                  "Scope 3 類別 9 下游運輸與配送，因貨物已脫離工廠控制",
                  "Scope 1,因運費由公司支付",
                  "Scope 3 類別 1 採購商品與服務"
            ],
            "correct": 0,
            "explain": "報告公司付費委外、將採購原料運入廠區之運輸屬於上游運輸配送;下游運輸(類 9)指產品售出後之運送。",
            "epRef": 61,
            "tag": [
                  "Scope3",
                  "運輸"
            ]
      },
      {
            "id": "L223_Q07",
            "type": "情境",
            "q": "某家電製造商販售冰箱予終端消費者,冰箱壽命約 15 年,使用期間持續消耗電力。針對該產品生命週期的排放盤查,下列哪一類別對此製造商最具重大性?",
            "options": [
                  "類別 1 採購商品與服務",
                  "類別 9 下游運輸與配送",
                  "類別 11 售出產品使用",
                  "類別 12 售出產品廢棄處理"
            ],
            "correct": 2,
            "explain": "耐久性用電產品在使用階段長期耗電,其累積排放通常遠超過製造與物流階段,屬下游類 11。",
            "epRef": 61,
            "tag": [
                  "Scope3",
                  "使用階段"
            ]
      },
      {
            "id": "L223_Q08",
            "type": "情境",
            "q": "某食品品牌商為擴展通路,與加盟主簽訂加盟契約,由加盟主自行營運門市,品牌商僅提供品牌授權與技術指導。加盟門市之用電排放對品牌商而言應歸入?",
            "options": [
                  "Scope 2",
                  "Scope 3 類別 13 下游租賃資產",
                  "Scope 3 類別 14 加盟店",
                  "不需納入,因加盟主為獨立法人"
            ],
            "correct": 2,
            "explain": "加盟店雖獨立營運,仍屬品牌方下游價值鏈,GHG Protocol 另設類別 14 專門歸屬其排放。",
            "epRef": 61,
            "tag": [
                  "Scope3",
                  "加盟"
            ]
      },
      {
            "id": "L223_Q09",
            "type": "情境",
            "q": "某公司首次進行 Scope 3 盤查,全公司採購品項逾 5000 項,供應商數量龐大且多數尚未提供碳足跡。公司希望快速建立第一年盤查基線,以下何種估算方法在資料可得性與推動時程考量下最為務實?",
            "options": [
                  "全面採用供應商特定法",
                  "全面採用支出導向法",
                  "全面採用數量導向法",
                  "僅盤查已具主要數據的品項,其餘略過"
            ],
            "correct": 1,
            "explain": "首次盤查且供應商數據不足時,以財務支出乘排放係數可快速完成全面性估算,後續再逐年改採更精準方法。",
            "epRef": 61,
            "tag": [
                  "估算方法"
            ]
      },
      {
            "id": "L223_Q10",
            "type": "情境",
            "q": "某半導體廠經第一年支出法盤查後,發現少數關鍵化學品供應商之排放佔 Scope 3 類 1 總量達 70%。第二年為提升精確度並兼顧推動成本,最合理的改善策略為?",
            "options": [
                  "全面改用支出法以維持一致性",
                  "關鍵供應商改採供應商特定法或數量法,其餘維持支出法",
                  "全品項一律改採供應商特定法以提升數據品質,不論該品項排放佔比高低",
                  "全品項改採預設國家平均值"
            ],
            "correct": 1,
            "explain": "混合法策略中,對重大排放源深化數據品質、次要項維持既有方法,可在效益與成本間取得平衡。",
            "epRef": 61,
            "tag": [
                  "估算方法",
                  "混合法"
            ]
      },
      {
            "id": "L223_Q11",
            "type": "情境",
            "q": "某企業規劃 Scope 3 盤查優先順序,考量時間與資源有限。依 GHG Protocol 對重大性的一般建議,下列三類排放多數製造業應優先視為熱點?",
            "options": [
                  "類 2 資本財、類 5 廢棄物、類 8 上游租賃",
                  "類 1 採購、類 4/9 運輸、類 11 產品使用",
                  "類 6 差旅、類 7 通勤、類 13 下游租賃",
                  "類 3 燃料與能源相關、類 14 加盟店營運、類 15 投資組合排放"
            ],
            "correct": 1,
            "explain": "多數製造業熱點集中於原料採購、上下游物流及產品使用階段,應納入首要盤查範疇。",
            "epRef": 61,
            "tag": [
                  "熱點分析"
            ]
      },
      {
            "id": "L223_Q12",
            "type": "情境",
            "q": "金融控股公司、家電製造商、連鎖餐飲品牌三者進行 Scope 3 重大性分析,其各自最顯著之排放類別最可能為何?",
            "options": [
                  "金控:類 15;家電:類 11;連鎖餐飲:類 1",
                  "金控:類 1 採購商品;家電:類 12 產品處置;連鎖餐飲:類 14 加盟",
                  "金控:類 6;家電:類 9;連鎖餐飲:類 7",
                  "金控:類 15;家電:類 1;連鎖餐飲:類 11"
            ],
            "correct": 0,
            "explain": "不同產業熱點差異顯著:金融業投資組合排放最大;耐久電器使用階段主導;餐飲業食材原料採購為主要來源。",
            "epRef": 61,
            "tag": [
                  "熱點分析",
                  "產業"
            ]
      },
      {
            "id": "L223_Q13",
            "type": "情境",
            "q": "某上市公司為系統化推動供應鏈減碳,規劃每年向關鍵供應商蒐集氣候資訊、設定減量目標並追蹤進度。下列何者是國際間最常被採用、且能與 CDP 主問卷銜接的供應鏈揭露機制?",
            "options": [
                  "ISO 14064-1 組織型盤查，因其涵蓋 Scope 3 揭露要求",
                  "CDP Supply Chain 計畫",
                  "GRI 標準",
                  "TCFD 架構"
            ],
            "correct": 1,
            "explain": "CDP Supply Chain 專為買方主導向供應商蒐集碳資訊而設計,與 CDP 企業揭露體系一致,廣為跨國企業採用。",
            "epRef": 61,
            "tag": [
                  "供應商管理",
                  "CDP"
            ]
      },
      {
            "id": "L223_Q14",
            "type": "情境",
            "q": "某採購部門依據供應商排放規模與合作重要性進行分級管理,資源投入以 A 級為主、B 級次之、C 級維持基本要求。下列對分級管理邏輯的描述何者最符合實務?",
            "options": [
                  "A 級應採供應商特定法、定期稽核;C 級可採支出法與問卷",
                  "所有級別一律要求提供產品碳足跡證書",
                  "A 級與 C 級供應商投入的稽核資源和數據要求應完全相同，以維持管理公平性",
                  "分級僅依供應商規模,不考慮排放熱點"
            ],
            "correct": 0,
            "explain": "核心高排放供應商應深化數據品質與稽核強度,次要供應商採較輕量方法以兼顧推動效益與資源配置。",
            "epRef": 61,
            "tag": [
                  "供應商管理",
                  "分級"
            ]
      },
      {
            "id": "L223_Q15",
            "type": "情境",
            "q": "某企業已建立碳資產管理系統(L222),清楚掌握自身 Scope 1/2 排放與各項減量專案產生之減量額度。該企業擬推動 Scope 3 供應鏈減碳,其碳資產管理能力對 Scope 3 工作最直接的助益為?",
            "options": [
                  "可直接將自身減量額度用以抵銷 Scope 3 排放,無須盤查",
                  "可協助識別內部減量經驗,作為輔導供應商減碳之參考與方法論移轉",
                  "可使 Scope 3 自動符合 SBTi 1.5°C 路徑",
                  "碳資產管理與 Scope 3 無關聯,須另建系統"
            ],
            "correct": 1,
            "explain": "內部累積之盤查方法、減量專案經驗與數據管理能力,可複製輔導供應商,並作為選商與分級依據,形成價值鏈綜效。",
            "epRef": 61,
            "tag": [
                  "整合",
                  "L222",
                  "碳資產"
            ]
      }
],
        "reinforcement": [
      {
            "id": "L223_R01",
            "type": "情境",
            "q": "某公司外購電力同時採用市場基礎法(market-based)與區位基礎法(location-based)進行 Scope 2 盤查,其用電量相同但兩法結果差異顯著。下列對此差異最合理的解釋為?",
            "options": [
                  "兩法計算公式不同且採用不同係數來源,結果必然不同且無法進行有意義的比較",
                  "市場基礎法反映所購買之電力產品(如再生能源憑證)排放係數,區位法反映電網平均",
                  "區位法僅適用於離岸企業",
                  "市場基礎法僅計算自發自用電力"
            ],
            "correct": 1,
            "explain": "市場基礎法採用契約或憑證對應之排放係數,可反映企業採購再生能源之減量;區位基礎法使用電網平均值。",
            "epRef": 61,
            "tag": [
                  "Scope2",
                  "market-based"
            ]
      },
      {
            "id": "L223_R02",
            "type": "情境",
            "q": "A 公司採購 B 公司生產之零組件,A 將該零組件之製造排放列為 Scope 3 類 1;B 公司同時將製造該零組件之排放列為自身 Scope 1。針對此現象下列敘述何者正確?",
            "options": [
                  "屬雙重計算錯誤,應由其中一方剔除",
                  "屬價值鏈盤查正常現象,非報告邊界內之雙重計算",
                  "A 公司應改列為 Scope 2",
                  "B 公司應將該零組件製造排放歸入 Scope 3 類別 11 售出產品使用"
            ],
            "correct": 1,
            "explain": "Scope 3 設計原意即讓排放在價值鏈上可被多方認列以驅動共同減量,不同組織邊界內不構成報告層級之雙重計算。",
            "epRef": 61,
            "tag": [
                  "雙重計算"
            ]
      },
      {
            "id": "L223_R03",
            "type": "情境",
            "q": "依數據品質金字塔之概念,下列對主要數據(primary)、次要數據(secondary)、預設值(default)之適用順序敘述何者最符合 GHG Protocol 建議?",
            "options": [
                  "優先使用預設值以簡化流程",
                  "關鍵重大排放源優先取得主要數據,次要項可用資料庫,低重要性項方用預設值",
                  "一律採用次要數據（產業資料庫）以維持各排放源間的計算一致性與可比較性",
                  "僅在查驗期才需要主要數據"
            ],
            "correct": 1,
            "explain": "數據品質應與排放重大性連動,熱點項目需供應商主要數據以降低不確定性,非熱點項可接受較低品質來源。",
            "epRef": 61,
            "tag": [
                  "數據品質"
            ]
      },
      {
            "id": "L223_R04",
            "type": "情境",
            "q": "某商業銀行盤查其投資組合排放,包含企業放款、股權投資與專案融資。依 GHG Protocol 與 PCAF 方法學,此類排放於該銀行應歸入?",
            "options": [
                  "Scope 1,因屬銀行核心業務",
                  "Scope 2,視同購入金融服務",
                  "Scope 3 類別 15 投資",
                  "不需納入,因非實體排放"
            ],
            "correct": 2,
            "explain": "金融機構之投融資活動衍生之受資方排放屬下游類 15,是金融業 Scope 3 最重大之類別,PCAF 提供分資產類別歸因方法。",
            "epRef": 61,
            "tag": [
                  "類15",
                  "投資",
                  "金融"
            ]
      },
      {
            "id": "L223_R05",
            "type": "情境",
            "q": "同為售出產品,A 公司銷售之耐久財為家用冷氣機,B 公司銷售之消耗品為洗髮精。兩者於類別 11(售出產品使用)之盤查特性差異,下列敘述何者最合適?",
            "options": [
                  "兩者計算方法完全相同,無差異",
                  "冷氣機需考量使用年限與耗電量累積,洗髮精使用階段排放相對微小且可能非熱點",
                  "洗髮精因年產量遠大於冷氣機,使用階段的總碳排放量必然高於冷氣機的耗電累積",
                  "耐久財不需納入類別 11"
            ],
            "correct": 1,
            "explain": "類 11 對耐久用電產品為高度重大(長期耗能累積),對一般消耗品則通常佔比有限,重大性分析結果將影響盤查深度。",
            "epRef": 61,
            "tag": [
                  "類11",
                  "使用階段"
            ]
      }
]
      }
    ]
  }
};
